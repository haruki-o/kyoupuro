...
void va_move(int h, int w, int outh, int outw) {
  ...
  queue<P> qu;
  qu.push({vh, vw});
  cout << "h,w " << h << " " << w << " out " << outh << " " << outw << endl;
  cout << "va_move (" << vh << ", " << vw << ") -> (" << h << ", " << w << ") "
       << endl;
  while (!qu.empty()) {
...
void set_parts(int h, int w, int si) {
  //(_h, _w) -> (h, w)
  cout << "h,w " << h << " " << w << " " << G2[h][w] << endl;
  cout << "si " << si << endl;
  auto [_h, _w] = best_parts(h, w, si);
  cout << "_h,_w " << _h << " " << _w << " " << _G[_h][_w] << endl;
  if (_h == h && _w == w) {
    _made_G[h][w] = 1;
    return;
  }
  ...
  cout << "_route[] " << endl;
  rep(i, 0, (int)_route.size()) cout << _route[i] << " ";
  cout << endl;
  move();
  _made_G[h][w] = 1;
  cout << "after_set_parts " << _ans << endl;
  out(_G);
}
  ...
  rep(k, 0, Q) {
    ...
    rep(j, 0, N - 3) {
      if (j % 2 == 0) {
        ...
        if (N - 1 - j == 4) {
          set_parts(N - 1, j + 4, 11);
          set_parts(N - 1, j + 3, 10);
          va_move(N - 1, j + 2, -1, -1);
          swap(_G[N - 1][j + 2], _G[N - 1][j + 3]);
          swap(_G[N - 1][j + 3], _G[N - 1][j + 4]);
          vh = N - 1, vw = j + 4;
          _made_G[N - 1][j + 4] = 0;
          _made_G[N - 1][j + 2] = 1;
          _ans += "RR";
        ...
        if (N - 1 - j == 4) {
          ...
          set_parts(0, j + 3, 10);
          va_move(0, j + 2, -1, -1);
          swap(_G[0][j + 2], _G[0][j + 3]);
          swap(_G[0][j + 3], _G[0][j + 4]);
          vh = 0, vw = j + 4;
          _made_G[0][j + 4] = 0;
          _made_G[0][j + 2] = 1;
          _ans += "RR";
    ...
    rep(i, 0, N - 3) {
      //縦3つ配置
      rep(j, 0, 3) set_parts(i + j, N - 3, 3 + j);
      //ずらす
      va_move(i, N - 2, -1, -1);
      swap(_G[i][N - 2], _G[i][N - 3]);
      swap(_G[i][N - 3], _G[i + 1][N - 3]);
      swap(_G[i + 1][N - 3], _G[i + 2][N - 3]);
      vh = i + 2;
      vw = N - 3;
      _made_G[i][N - 2] = 1;
      _made_G[i + 2][N - 3] = 0;
