...
struct Solver {
  ...
  double all_move_time = 0.0;
  double all_connect_time = 0.0;
  double all_forest_time = 0.0;
  ...
  P forest_connect() {
    ...
    rep(c, 1,2) {
      ...
      // P mid_pos = {-1, -1};
      P to_pos = {-1, -1};
      MoveAction ret = MoveAction(0, 0, 0, 0);
      ...
      if ((int)select_connects[c].empty())
        from_forest[select_seeds[c]] = 1;
      //中継点を探す
      queue<tuple<int, int, int, int,int, MoveAction>> qu;
        ...
        rep(dir, 0, 4) {
          ...
          while (can_move(ner, nec, dir)) {
            ...
            rep(_dir, 0, 4) {
              if (_dir % 2 == dir % 2)
                continue;
              if (!can_move(ner, nec, _dir))
                continue;
              int tor = ner + DR[_dir];
              int toc = nec + DC[_dir];
              if (select_fields[c][tor][toc] == '0') {
                qu.push({m.first.first, m.first.second, ner, nec, dir,MoveAction(ner,nec,tor,toc)});
                break;
              }
            }
            break;
          ...
          while (can_move(ner, nec, dir)) {
            ...
            if (select_fields[c][ner][nec] == USED) {
              if (!from_forest.count({ner, nec}) &&
                ...
                int after_size = (int)select_connects[c].size()+ to_connect_size;
                // cerr << "find : " << (int)select_connects[c].size() << " "
                //      << to_connect_size << " " << c << endl;
                if (_score < after_size * (after_size - 1) / 2) {
                  _score = after_size * (after_size - 1) / 2;
                  from_pos = {get<0>(cu), get<1>(cu)};
                  // mid_pos = {get<2>(cu), get<3>(cu)};
                  to_pos = {ner, nec};
                  ret = get<5>(cu);
                }
              }
      ...
      if (max_score < _score) {
        ...
        if (from_pos.first != -1) {
          select_connects[c].push_back(ConnectAction(
              from_pos.first, from_pos.second, to_pos.first, to_pos.second));
          select_moves[c].push_back(ret);
        }
      }
      // cerr << c << " " << _score << endl;
    }
  ...
  Result solve() {
    ...
    while (1) {
      ...
      if (cu.first == -1)
        continue;
      cerr << cu.second << " " << (int)select_connects[cu.first].size() << endl;
      cerr << "pos :" << select_seeds[cu.first].first << " "
           << select_seeds[cu.first].second << endl;
    ...
    cerr << "soleve_roop : " << solve_roop << " " << (int)ret1.size() << " "
         << (int)ret2.size() << endl;
...
int main() {
  ...
  inisial_field = field;
  int score = 0;
  vector<MoveAction> _ans_move;
  vector<ConnectAction> _ans_connect;
  ...
  rep(i, 0, 1) {
    ...
    while (1) {
      ...
      // cerr << i << "ケース" << endl;
      // cerr << s.all_move_time << endl;
      // cerr << s.all_connect_time << endl;
      // cerr << s.all_forest_time << endl;
    ...
    int at_score = calc_score(N, inisial_field, Result(ans_move, ans_connect));
    // cerr << "Score = " << at_score << endl;
    if (score < at_score) {
      score = at_score;
      _ans_move = ans_move;
      _ans_connect = ans_connect;
    }
  }
  print_answer(Result(_ans_move, _ans_connect));
  cerr << calc_score(N, inisial_field, Result(_ans_move, _ans_connect)) << endl;
  return 0;
}

// 216941が元
// bfs()をO(100 * N ^2) -> O(100 * 5 * 5)
// main()内で10回回すために計算量改善した