          ...
          while (can_move(ner, nec, dir)) {
            ...
            if (select_fields[c][ner][nec] == USED) {
              if (!from_forest.count({ner, nec}) &&
                ...
                //-1 は中継点分
                int after_size =
                    (int)select_connects[c].size() + 1 + to_connect_size - 1;
  ...
  Result solve() {
    ...
    while (1) {
      ...
      if (time > TIME_LIMIT)
        break;
      solve_roop++;
      auto start_move_Clock = system_clock::now();
      move((int)(action_count_limit / (double)2.2));
      all_move_time +=
          duration_cast<microseconds>(system_clock::now() - start_move_Clock)
              .count() *
          1e-6;

      // cerr << "after move()" << endl;
      auto start_connect_Clock = system_clock::now();
      connect();
      all_connect_time +=
          duration_cast<microseconds>(system_clock::now() - start_connect_Clock)
              .count() *
          1e-6;

      // cerr << "after connect()" << endl;
      auto start_forest_Clock = system_clock::now();
      P cu = forest_connect();
      all_forest_time +=
          duration_cast<microseconds>(system_clock::now() - start_connect_Clock)
              .count() *
