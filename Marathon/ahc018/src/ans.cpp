...
struct PredictBan {
  ...
  void update_ban() {
    ...
    rep(i, 0, (ll)se_x.size()) {
      rep(j, 0, (ll)se_y.size() - 1) {
        ...
        ll unit = (val2 - val1) / (se_y[j + 1] - se_y[j]);
        rep(k, se_y[j], se_y[j + 1]) {
          ban[k][se_x[i]] = val1 + unit * (k - se_y[j]);
        }
      }
    }
    rep(i, 0, (ll)se_y.size()) {
      rep(j, 0, (ll)se_x.size() - 1) {
        ...
        ll unit = (val2 - val1) / (se_x[j + 1] - se_x[j]);
        rep(k, se_x[j], se_x[j + 1]) {
          ban[se_y[i]][k] = val1 + unit * (k - se_x[j]);
        }
      }
    }
    // ここまで格子状
    rep(i, 0, 200) {
      rep(j, 0, (ll)se_x.size() - 1) {
        ll val1 = ban[i][se_x[j]];
        ll val2 = ban[i][se_x[j + 1]];
        ll unit = (val2 - val1) / (se_x[j + 1] - se_x[j]);
        rep(k, se_x[j], se_x[j + 1]) {
          ban[i][k] = val1 + unit * (k - se_x[j]);
        }
      }
    }
    rep(i, 0, 200) {
      rep(j, 0, (ll)se_y.size() - 1) {
        ...
        ll unit = (val2 - val1) / (se_y[j + 1] - se_y[j]);
        rep(k, se_y[j], se_y[j + 1]) {
          ban[k][i] = (ban[k][i] + val1 + unit * (k - se_y[j])) / 2;
        }
      }
    }
  }
  ...
  PredictBan(ll flag) {
    if (C <= 8) {
      rep(i, 0, 2) predict_border.push_back(50);
    } else {
      rep(i, 0, 1) predict_border.push_back(100);
    }

    ban.resize(200);
    rep(i, 0, 200) ban[i].assign(200, 0);
    rep(_y, 0, 200) if (_y % 20 == 0 || _y == 199) se_y.push_back(_y);
    rep(_x, 0, 200) if (_x % 20 == 0 || _x == 199) se_x.push_back(_x);
    map<P, int> skip;
  ...
  // flag := 0 ほんばん, 1 local
  void add_PredictBan(ll flag, ll num) {
    predict_border.clear();
    if (C <= 8) {
      rep(i, 0, 2) predict_border.push_back(50);
    } else {
      rep(i, 0, 1) predict_border.push_back(100);
...
void local_unit_mining(ll _y, ll _x, PredictBan &p_ban, ll &pre) {
  if (breaked.count({_y, _x}))
    return;
  ll set_power = pre;
  if (pre == -1)
    set_power = min((ll)500, p_ban.ban[_y][_x]);
  if (C <= 8)
    set_power /= 10;
  else if (C <= 16)
    set_power /= 5;
...
void unit_mining(ll _y, ll _x, PredictBan &p_ban, ll &pre) {
  if (breaked.count({_y, _x}))
    return;
  ll set_power = pre;
  if (pre == -1)
...
struct Graph {
  ...
  void break_load(ll cu, ll to, PredictBan &p_ban) {
    ...
    ll pre = -1;
    while (1) {
      local_unit_mining(at_y, at_x, p_ban, pre);
      // unit_mining(at_y, at_x, p_ban, pre);
  ...
  void unit_break_load(ll cu, ll to, PredictBan &p_ban) {
    vvll dp(200, vll(200, -1));
    vvll dir(200, vll(200, -1));
    priority_queue<T, vT, greater<T>> pq;
    pq.push({0, y[cu] * 200 + x[cu], -1});
    ...
    rep(i, 0, (ll)_mining.size() / 20) {
      ll idx = (ll)rand() % (ll)_mining.size();
      local_unit_mining(_mining[idx].first, _mining[idx].second, p_ban, pre);
      // unit_mining(at_y, at_x, p_ban, pre);
      ll val = p_ban.ban[_mining[idx].first][_mining[idx].second];
      if (val > 1000) {
        rep(i, -8, 9) {
          rep(j, -8, 9) {
            ll to_y = _mining[idx].first + i;
            ll to_x = _mining[idx].second + j;
            if (to_y < 0 || to_y >= 200 | to_x < 0 || to_x >= 200)
              continue;
            p_ban.ban[to_y][to_x] = 5000;
          }
        }
      }
    }
  ...
  void predict_break_load(ll cu, ll to, PredictBan &p_ban, vvll &seen) {
    vvll dp(200, vll(200, -1));
    vvll dir(200, vll(200, -1));
    priority_queue<T, vT, greater<T>> pq;
    pq.push({0, y[cu] * 200 + x[cu], -1});
    ...
    ll at_x = x[to];
    while (1) {
      score_predict += p_ban.ban[at_y][at_x] + 2 * C;
      seen[at_y][at_x] = 1;
      ll _to = dir[at_y][at_x];
      if (_to == -1)
        break;
  ...
  void predict_dijkstra(PredictBan &p_ban) {
    vll dp(N, -1);
    vvll seen(200, vll(200, 0));
    priority_queue<T, vT, greater<T>> pq;
    for (ll _sw : sw)
      pq.push({(ll)0, _sw, -1});
    ...
    while (!pq.empty()) {
      ...
      dp[cu] = cu_cost;
      if (origin != -1) {
        predict_break_load(cu, origin, p_ban, seen);
      }
  ...
  void dijkstra(PredictBan &p_ban) {
    ...
    while (!pq.empty()) {
      ...
      dp[cu] = cu_cost;
      if (origin != -1) {
        unit_break_load(cu, origin, p_ban);
...
int main() {
  ...
  G.predict_dijkstra(p_ban);
  ll num = 0;

  if (score_predict > 200000)
    num = 3;
  else if (score_predict > 50000)
    num = 1;
  rep(i, 1, num + 1) p_ban.add_PredictBan(1, i);
  G.dijkstra(p_ban);
...
// local7
// 予測が200,000超えた時追加で予測します(1.024倍)

// 提出の時
// PredictBan p_ban(1)の1 -> 0に変更
// add_PredictBan p_ban(1,10)の1 -> 0に変更
// local_unit_mining -> unit_mining
// Sの入力を消す