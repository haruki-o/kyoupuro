...
struct Graph {
  ...
  void break_load(ll fr_y, ll fr_x, ll to_y, ll to_x, vP &break_rocks) {
    ll cu_y = fr_y;
    ll cu_x = fr_x;
    ll _dy = to_y - cu_y;
    ll _dx = to_x - cu_x;
    ...
    if (!breaked.count({cu_y, cu_x}))
      break_rocks.push_back({cu_y, cu_x});
    // breaked[{cu_y, cu_x}] = 1;
    if (pred1 > pred2) {
      rep(i, 0, (ll)abs(_dy)) {
        cu_y += _dy / abs(_dy);
  ...
  void dijkstra(vP &break_rocks) {
    ...
    while (!pq.empty()) {
      ...
      if (origin != -1) {
        break_load(cu_y, cu_x, origin / 200, origin % 200, break_rocks);
      }
      vll ava = houses;
      // ava.insert(ava.end(), sw.begin(), sw.end());
      for (ll to : houses) {
        if (dp[y[to]][x[to]] != -1)
          continue;
        ll dist = abs(cu_y - y[to]) + abs(cu_x - x[to]);
        pq.push({dist, y[to] * 200 + x[to], cu});
      }
    }
  }

  void prim(ll sta_sw, vll _houses, vP &break_rocks) {
    ...
    qu.push(ma_s[sta_sw]);
    seen[ma_s[sta_sw]] = 1;
    break_rocks.push_back({y[ma_s[sta_sw]], x[ma_s[sta_sw]]});
