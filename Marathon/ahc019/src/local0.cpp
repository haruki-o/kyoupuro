...
void modify_break(Cube &nec_c, Cube &sub_c, vector<Part> &parts) {
  int break_idx = rand() % (int)parts.size();
  parts.erase(parts.begin() + break_idx);
  Cube fix_c;
  for (Part cu : parts) {
    for (Point cu_p : cu.b)
...
void solve(Cube &nec_c1, Cube &sub_c1, Cube &nec_c2, Cube &sub_c2,
  ...
  while (1) {
    ...
    sub_c1.update(nec_c1);
    sub_c2.update(nec_c2);
    // 修復する
