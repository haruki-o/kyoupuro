...
#define TIME_LIMIT1 (55)
#define TIME_LIMIT2 (57)
...
void solve(Cube &nec_c1, Cube &sub_c1, Cube &nec_c2, Cube &sub_c2,
  ...
  while (1) {
    ...
    // 大きさmin(10,D)以上のブロックが孤立していたらダメ
    if ((int)parts1.size() != (int)parts2.size()) {
      rep(i, 0, (int)parts1.size()) {
        if (min(10,D) <= parts1[i].vol) {
          rep(j, 0, (int)parts2.size()) if (b_idx2[j] != i + 1) flag = 0;
        }
      }
      rep(j, 0, (int)parts2.size()) {
        if (min(10,D) <= parts2[j].vol && (int)parts1.size() < b_idx2[j])
          flag = 0;
      }
    }
...
// 同形伸ばすパーツで、
// 初め(0.2秒)大きさ1,2,3は伸ばして、他縮める(大きいのは効果量が少なく、場所も取る)
// 残り(0.2秒)は制限なしで伸ばす

// local7の２つのobjectが同じパーツ数で、余剰パーツがある時更新しません(すべてのDにしました)

// print()がおかしいので直しました

// ２つのobjectのバーツ数が違って、大きさmin(10,D)以上の非共通パーツがある時更新しません