...
#define TIME_LIMIT4 (0.2)
#define TIME_LIMIT1 (3.0)
#define TIME_LIMIT2 (5.5)
#define TIME_LIMIT3 (5.6)
#define TIME_LIMIT (5.9)
#define SAME_EXTEND_TIME_LIMIT (0.010)
#define def (20101)
...
ll diff_calc_score(vector<Part> &parts1, vector<Part> &parts2, vi &b_idx1,
  ...
  if (type == 1) {
    ...
    b_idx2[ch_idx] = 0;
    diff_calc_score(parts1, parts2, b_idx1, b_idx2, 2);
  }
...
void modify(Cube &nec_c, Cube &sub_c, vector<Part> &parts, vi &b_idx) {
  int swi = rand() % 2;
...
void modify_create(Cube &nec_c, Cube &sub_c, vector<Part> &parts) {
  Point seed(-1, -1, -1);
  Cube fix_c;
  for (Part cu : parts) {
    for (Point cu_p : cu.b)
  ...
  rep(_x, 0, D) {
    ...
    rep(_y, 0, D) {
      ...
      rep(_z, 0, D) {
        z = (z + 1) % D;
        if (fix_c.b[x][y][z] == 0 && nec_c.b[x][y][z] == 1)
          seed = Point(x, y, z);
      }
    }
  }
  ...
  queue<Point> candi;
  candi.push(seed);
  fix_c.b[seed.x][seed.y][seed.z] = 1;
  cu_part.b.push_back(seed);
  cu_part.points.push_back(seed);
  int upper = rand() % 5;
  int loop_num = 0;
  while (!candi.empty()) {
    if (loop_num == upper)
    ...
    rep(dir, 0, 6) {
      ...
      if (!nec_c.b[to.x][to.y][to.z])
        continue;
      fix_c.b[to.x][to.y][to.z] = 1;
      cu_part.b.push_back(to);
      cu_part.points.push_back(to);
      cu_part.vol++;
      candi.push(to);
    }
  }

  // cerr << endl << endl << (int)cu_part.points.size() << endl << endl;
  Sort(cu_part.points);
  parts.push_back(cu_part);

  while (1) {
    int flag = 1;
    int i = rand() % (int)parts.size();
    rep(_i, 0, (int)parts.size()) {
      i = (i + 1) % (int)parts.size();
      queue<Point> qu;
      int upper = rand() % 3 + 1;
      for (Point _p : parts[i].b)
        qu.push(_p);
      while (!qu.empty()) {
        rep(dir, 0, 6) {
          if (upper == 0)
          ...
          if (!nec_c.b[to.x][to.y][to.z])
            continue;
          fix_c.b[to.x][to.y][to.z] = 1;
          parts[i].b.push_back(to);
          parts[i].points.push_back(to);
          parts[i].vol++;
          upper--;
          flag = 0;
          qu.push(to);
...
int same_num(vi &b_idx1, vi &b_idx2) {
  int sum = 0;
  rep(i, 0, (int)b_idx2.size()) {
    if (b_idx2[i] <= (int)b_idx1.size())
      sum++;
  }
  return sum;
}

bool is_satisfy_silhouette(Cube &nec_c, Cube &sub_c, vector<Part> &parts) {
  Cube fix_c;
  for (Part cu : parts) {
    for (Point cu_p : cu.b)
      fix_c.b[cu_p.x][cu_p.y][cu_p.z] = 1;
...
void print(vector<Part> parts1, vector<Part> parts2) {
  ...
  rep(j, 0, (ll)parts2.size()) {
    ...
    // rep(i, 0, (ll)parts1.size()) {
    //   if (same_part(parts1[i], parts2[j]))
    //     cout << i + 1 << " " << same_idx << endl;
    //   else {
    //     cout << "out " << i + 1 << " " << same_idx << endl;
    //     if (parts1[i].vol == parts2[j].vol) {
    //       for (Point cu : parts1[i].points)
    //         cout << "(" << cu.x << ", " << cu.y << ", " << cu.z << ")";
    //       cout << endl;
    //       for (Point cu : parts2[j].points)
    //         cout << "(" << cu.x << ", " << cu.y << ", " << cu.z << ")";
    //       cout << endl;
    //     }
    //   }
...
void print_parts_size(vector<Part> &parts) {
  map<int, int> ma;
  for (Part cu : parts)
    ma[cu.vol]++;
  rep(i, 1, 10) cerr << ma[i] << " ";
  cerr << endl;
}
...
void solve(Cube &nec_c1, Cube &sub_c1, Cube &nec_c2, Cube &sub_c2,
  ...
  int mi = 100;
  while (1) {
    if (TIME_LIMIT4 < ela_times(startClock))
      break;
    vector<Part> _parts1, _parts2;
    init_all_parts(nec_c1, sub_c1, _parts1);
    init_all_parts(nec_c2, sub_c2, _parts2);
    if (chmin(mi, abs((int)_parts1.size() - (int)_parts2.size()))) {
      parts1 = _parts1;
      parts2 = _parts2;
    }
  }
  ...
  while (1) {
    if (TIME_LIMIT1 < ela_times(startClock))
      break;

    // 作る or 壊す・
    if (parts1.size() != parts2.size() && rand() % 5 == 0) {
      if (parts1.size() < parts2.size())
        modify_create(nec_c1, sub_c1, parts1);
      else
        modify_create(nec_c2, sub_c2, parts2);
    } else {
      if (parts1.size() >= parts2.size()) {
        int i = rand() % (int)parts1.size();
        rep(_i, 0, (int)parts1.size()) {
          i = (i + 1) % (int)parts1.size();
          if (modify_break(nec_c1, sub_c1, parts1, i))
            break;
        }
      }
      if (parts1.size() < parts2.size()) {
        ...
        rep(_i, 0, (int)parts2.size()) {
          ...
          if (modify_break(nec_c2, sub_c2, parts2, i))
            break;
        }
      }
    }
    ...
    bool flag = 1;
    if ((int)parts1.size() == (int)parts2.size() &&
        same_num(b_idx1, b_idx2) != (int)parts1.size())
      flag = 0;
    if (35 < min((int)parts1.size(), (int)parts2.size()))
      flag = 1;
    if (all_best_score == 1e15)
      flag = 1;

    // 成功
    if (chmin(all_best_score, best_score) && flag) {
      all_best_parts1 = parts1;
      all_best_parts2 = parts2;
      print_num(calc_score(parts1, parts2));
      cerr << " ";
      cerr << (int)parts1.size() << " " << (int)parts2.size() << " ";
      cerr << same_num(b_idx1, b_idx2) << " ";
      cerr << endl;
      print_parts_size(parts1);
      print_parts_size(parts2);
      cerr << endl;
    } else {
  ...
  // 壊す・同形伸ばし
  while (1) {
    if (TIME_LIMIT2 < ela_times(startClock))
      break;
    // 壊す
    vector<Part> _parts1, _parts2;
    _parts1 = parts1;
    _parts2 = parts2;
    int break_idx1 = rand() % (int)_parts1.size();
    int break_idx2 = -1;
    int i = rand() % (int)_parts2.size();
    rep(_i, 0, (int)_parts2.size()) {
      i = (i + 1) % (int)_parts2.size();
      if (same_part(_parts1[break_idx1], _parts2[i])) {
        break_idx2 = i;
        break;
      }
    }
    if (break_idx2 == -1)
      continue;
    _parts1.erase(_parts1.begin() + break_idx1);
    _parts2.erase(_parts2.begin() + break_idx2);

    // 伸ばす
    auto __startClock = system_clock::now();
    while (1) {
      if (SAME_EXTEND_TIME_LIMIT < ela_times(__startClock))
        break;

      int idx1 = rand() % (int)_parts1.size();
      int idx2 = -1;
      int i = rand() % (int)_parts2.size();
      rep(_i, 0, (int)_parts2.size()) {
        i = (i + 1) % (int)_parts2.size();
        if (same_part(_parts1[idx1], _parts2[i])) {
          idx2 = i;
          break;
        }
      }
      if (idx2 == -1)
        continue;
      vector<Part> __parts1, __parts2;
      __parts1 = _parts1;
      __parts2 = _parts2;
      __parts1[idx1].extend(nec_c1, sub_c1, _parts1);
      __parts2[idx2].extend(nec_c2, sub_c2, _parts2);

      if (same_part(__parts1[idx1], __parts2[idx2])) {
        _parts1 = __parts1;
        _parts2 = __parts2;
      }
    }

    if (is_satisfy_silhouette(nec_c1, sub_c1, _parts1) &&
        is_satisfy_silhouette(nec_c2, sub_c2, _parts2)) {
      parts1 = _parts1;
      parts2 = _parts2;
      print_num(calc_score(parts1, parts2));
      cerr << " ";
  ...
  while (1) {
    ...
    if (TIME_LIMIT < ela_times(startClock))
      break;
    if (TIME_LIMIT3 < ela_times(startClock)) {
      if (upper == 6) {
        cerr << "mid" << endl;
        print_parts_size(parts1);
        print_parts_size(parts2);
    ...
    rep(_i, 0, (int)parts2.size()) {
      i = (i + 1) % (int)parts2.size();
      if (same_part(parts1[idx1], parts2[i])) {
        idx2 = i;
        break;
      }
    }
...
int main() {
  ...
  cerr << "stat_loop_num : " << stat_loop_num << endl;
}

// local17
// 基本はlocal16

// 同じパーツ数で非共通パーツがあるとき、(int)parts1.size() < 35の時更新しません
// 最後の共通パーツを延ばすupperを6に変更しました
