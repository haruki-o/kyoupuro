...
bool common_parts_HC(Cube &nec_c1, Cube &sub_c1, Cube &nec_c2, Cube &sub_c2,
                     vector<Part> &parts1, vector<Part> &parts2, vi &b_idx1,
                     vi &b_idx2) {
  int loop_num = 0;
  int pre_update = -1;
  int flag = 0;
  while (1) {
    ...
    int idx2;
    rep(i, 0, (int)parts1.size()) if (idx1 + 1 == b_idx2[i]) idx2 = i;
...
bool is_object_same_parts(vector<Part> &parts1, vector<Part> &parts2) {
  if (parts1.size() == parts2.size()) {
    vi b_idx1((int)parts1.size(), 0);
    vi b_idx2((int)parts2.size(), 0);
    diff_calc_score(parts1, parts2, b_idx1, b_idx2, 0);
    vi seen((int)parts1.size() + 1, 0);
    for (int idx : b_idx2) {
    ...
    for (int idx : seen)
      sum += idx;
    if (sum == (int)parts2.size())
      return true;
  }
  return false;
}

void extend_shrink_HC(Cube &nec_c1, Cube &sub_c1, Cube &nec_c2, Cube &sub_c2,
  ...
  while (1) {
    ...
    while (1) {
      if (all_best_score == 1e15)
        stat_loop_num++;
      if (0.001 < ela_times(_startClock) || mi_diff == 0)
        break;
      vector<Part> _parts1, _parts2;
      init_all_parts(nec_c1, sub_c1, _parts1);
      init_all_parts(nec_c2, sub_c2, _parts2);
      ll score = abs((int)_parts1.size() - (int)_parts2.size());
      if (chmin(mi_diff, score)) {
        parts1 = _parts1;
        parts2 = _parts2;
      }
    }

    // 壊す・同形伸ばし or 壊す・伸縮の山登り
    int flag = is_object_same_parts(parts1, parts2);
    ll best_score = 1e15;
    _startClock = system_clock::now();
    while (1) {
      if (0.5 < ela_times(_startClock))
        break;
      if (flag) {
        vector<Part> _parts1, _parts2;
        _parts1 = parts1;
        ...
        vi b_idx1((int)parts1.size(), 0);
        vi b_idx2((int)parts2.size(), 0);
        best_score = diff_calc_score(parts1, parts2, b_idx1, b_idx2, 0);
        // 壊す
        int break_idx1 = rand() % (_parts1.size() - 1);
        int break_idx2;
        rep(i, 0, (int)_parts1.size()) {
          if (break_idx1 + 1 == b_idx2[i])
            break_idx2 = i;
        }
        _parts1.erase(_parts1.begin() + break_idx1);
        _parts2.erase(_parts2.begin() + break_idx2);
        b_idx1.erase(b_idx1.begin() + break_idx1);
        b_idx2.erase(b_idx2.begin() + break_idx2);
        rep(i, 0, (int)b_idx1.size()) b_idx1[i] = i + 1;
        rep(i, 0, (int)b_idx2.size()) if (break_idx1 < b_idx2[i]) b_idx2[i]--;
        // 共通パーツが延ばせないか山登り
        if (!common_parts_HC(nec_c1, sub_c1, nec_c2, sub_c2, _parts1, _parts2,
                             b_idx1, b_idx2))
          break;
        if (is_satisfy_silhouette(nec_c1, sub_c1, _parts1) &&
            is_satisfy_silhouette(nec_c2, sub_c2, _parts2)) {
          ll score = calc_score(_parts1, _parts2);
          if (chmin(best_score, score)) {
            parts1 = _parts1;
            parts2 = _parts2;
          }
        ...
        _parts2 = parts2;
        // 壊す
        if (_parts1.size() >= _parts2.size()) {
          while (!modify_break(nec_c1, sub_c1, _parts1)) {
            if (TIME_LIMIT < ela_times(startClock))
              break;
          }
        }
        if (TIME_LIMIT < ela_times(startClock))
          break;
        if (_parts1.size() < _parts2.size()) {
          while (!modify_break(nec_c2, sub_c2, _parts2)) {
            if (TIME_LIMIT < ela_times(startClock))
              break;
          }
        }
        auto stat_startClock = system_clock::now();
        extend_shrink_HC(nec_c1, sub_c1, nec_c2, sub_c2, _parts1, _parts2);
        stat_time += ela_times(stat_startClock);
        ll score = calc_score(_parts1, _parts2);
        if (chmin(best_score, score)) {
          parts1 = _parts1;
          parts2 = _parts2;
          flag = is_object_same_parts(parts1, parts2);
        }
      }
    }
    // print_num(best_score);
    // cerr << " ";
    // print_num(all_best_score);
    // cerr << endl;
...
// local2
// local0に加え
// 2つのオブジェクトの数が異なった時、多い方を削除します
// 2つのオブジェクトの数が同じで、パーツの大きさも同じとき、伸ばします