...
void solve(Cube &nec_c1, Cube &sub_c1, Cube &nec_c2, Cube &sub_c2,
  ...
  while (1) {
    ...
    while (1) {
      ...
      // if (chmax(ma1, (int)_parts1.size()))
      //   parts1 = _parts1;
      // if (chmin(ma2, (int)_parts2.size()))
      //   parts2 = _parts2;
    }
    ...
    int mi = min((int)parts1.size(), (int)parts2.size());
    while (1) {
      if (0.1 < ela_times(_startClock))
        break;
      if ((int)parts1.size() == (int)parts2.size() && (int)parts1.size() == mi)
        break;
      // 壊す
      if (parts1.size() >= parts2.size())
        modify_break(nec_c1, sub_c1, parts1);
      else
        modify_break(nec_c2, sub_c2, parts2);
    }
    print(parts1, parts2);
    cerr << (int)parts1.size() << " " << (int)parts2.size() << " ";
    // 伸縮の山登り(同形固定)
    ...
    while (1) {
      ...
      if (rand() % 2) {
        ...
        stat_time += ela_times(__startClock);
...
// local3
// 同じパーツ数になるようにパーツを壊して伸ばす
// 同形パーツを固定してその他で伸縮させる
// あまりよくなさそう

// 縮める時条件を満たさないでも、推移させる
// 最後に同形のまま伸ばして条件を満たすか
// 全然条件を満たさないのでなし