...
void select_part(Cube &nec_c, Cube &sub_c, vector<Part> &parts, Part &part) {
  Cube fix_c;
  for (Part _part : parts) {
    for (Point cu : _part.b)
      fix_c.b[cu.x][cu.y][cu.z] = 1;
  }
  int x = rand() % D;
  int y = rand() % D;
  ...
  rep(_x, 0, D) {
    ...
    rep(_y, 0, D) {
      ...
      rep(_z, 0, D) {
        z = (z + 1) % D;
        if (nec_c.b[x][y][z] == 1 && fix_c.b[x][y][z] == 0) {
          Point cu(x, y, z);
          part.b.push_back(cu);
          part.points.push_back(cu);
          return;
        }
      }
    }
...
void solve(Cube &nec_c1, Cube &sub_c1, Cube &nec_c2, Cube &sub_c2,
  ...
  while (1) {
    ...
    parts2.clear();
    while (1) {
      if (is_satisfy_silhouette(nec_c1, sub_c1, parts1) ||
          is_satisfy_silhouette(nec_c2, sub_c2, parts2))
        break;
      if (TIME_LIMIT < ela_times(startClock))
        break;
      // cerr << (int)parts1.size() << " " << (int)parts2.size() << endl;
      Part part1, part2;
      select_part(nec_c1, sub_c1, parts1, part1);
      select_part(nec_c2, sub_c2, parts2, part2);
      ...
      while (1) {
        ...
        _parts1 = parts1;
        _parts2 = parts2;
        _parts1.back().extend(nec_c1, sub_c1, _parts1);
        _parts2.back().extend(nec_c2, sub_c2, _parts2);
        if (same_part(_parts1.back(), _parts2.back()) &&
            (int)parts1.back().b.size() != (int)_parts1.back().b.size()) {
          pre_update = loop_num;
          parts1 = _parts1;
          parts2 = _parts2;
          size++;
    ...
    if (is_satisfy_silhouette(nec_c1, sub_c1, parts1) &&
        is_satisfy_silhouette(nec_c2, sub_c2, parts2)) {
      ll score = calc_score(parts1, parts2);
      if (chmin(all_best_score, score)) {
