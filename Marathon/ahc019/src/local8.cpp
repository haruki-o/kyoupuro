...
void solve(Cube &nec_c1, Cube &sub_c1, Cube &nec_c2, Cube &sub_c2,
  ...
  while (1) {
    ...
    if (TIME_LIMIT < ela_times(startClock))
      break;
    // 壊す or 作る
    bool modify_create_flag = 0;
    if (rand() % 10 != 0) {
      int flag = 0;
      if (parts1.size() > parts2.size())
        flag = 1;
      if (parts1.size() == parts2.size() && rand() % 2)
        flag = 1;
      if (flag) {
    ...
    if (modify_create_flag) {
      cerr << endl;
      continue;
    }

    bool flag =
        (int)parts1.size() == (int)parts2.size() && _sum != (int)parts1.size()
            ? 0
            : 1;
...
// local8
// 基本はlocal7
// 壊すに加えて新たに作成するようにもしました
// 壊すは、数が同じときどっちかorどっちもをランダムで選びます
// 作成するは,5回に1回作成し、作成後は、all_best_scoreは更新せず、次に壊してスコアが更新したときに、初めて採用されます