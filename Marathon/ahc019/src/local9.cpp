...
ll diff_calc_score(vector<Part> &parts1, vector<Part> &parts2, vi &b_idx1,
  ...
  if (type == 1) {
    ...
    rep(i, 0, (ll)parts1.size()) {
      if (b_idx1[i] != 0)
        continue;
      int j = rand() % (int)parts2.size();
      int change_idx = -1;
      rep(_j, 0, (ll)parts2.size()) {
        j = (j + 1) % (int)parts2.size();
        if (same_part(parts1[i], parts2[j])) {
          change_idx = j;
          if (b_idx2[j] == 0)
            break;
      ...
      if (change_idx != -1)
        b_idx2[change_idx] = i + 1;
    }
  }
  // parts2が変更された(parts2の1つの要素が0になってる)
  if (type == 2) {
    vi used((int)parts1.size() + 1, -1);
    rep(i, 0, (ll)parts2.size()) {
      if (b_idx2[i] <= (int)parts1.size() && b_idx2[i] != 0)
        used[b_idx2[i]] = i;
    }
    rep(j, 0, (ll)parts2.size()) {
      if (b_idx2[j] != 0)
      ...
      if (change_idx != -1) {
        b_idx2[change_idx] = change_val;
        if (used[change_val] != -1)
          b_idx2[used[change_val]] = 1e5;
      }
    }
    rep(j, 0, (ll)parts2.size()) {
      if ((int)parts1.size() < b_idx2[j])
  ...
  while (1) {
    ...
    while (1) {
      ...
      if (rand() % 2) {
        ...
        modify(nec_c1, sub_c1, _parts1, _b_idx1, idx);
        ...
        if (ava_idx.empty())
          continue;
        int idx = ava_idx[rand() % (int)ava_idx.size()];
        modify(nec_c2, sub_c2, _parts2, _b_idx2, idx);
...
// 修復パートで、同形のものは選択しない
// same_num() : 同形パーツの数
// diff_calc_score() : b_idxの仕様を変更しました