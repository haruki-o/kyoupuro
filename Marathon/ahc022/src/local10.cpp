...
struct Solver_200_P {
  ...
  const double sa_place_time = 0.5;
  const double create_P_time = 3.5;
  ll Success_num = 1;
  ll Bio_N;
  ll Bio_K;
  ...
  void new_moves(vvll P, vector<Pos> &ret_moves, vll wholes, vll exits,
                 ll &_m_N) {
    // cerr << wholes.size() << " " << exits.size() << endl;
    _m_N = 1;
    if (3 <= exits.size()) _m_N = 2;
    if (5 <= exits.size()) _m_N = 3;

    map<pll, int> ma;
    vector<Pos> all_moves;
    Pos origin(0, 0);
    ll am = L * L / 10;
    rep(i, 0, L) {
      rep(j, 0, L) {
        if ((i * L + j) % am != 0) continue;
        Pos at(i, j);
        Pos _move = move(origin, at);
        if (!ma.count({_move.y, _move.x})) {
          all_moves.push_back(_move);
          ma[{_move.y, _move.x}] = 1;
        }
      }
    }

    ll all_N = (ll)all_moves.size();
    cerr << all_N << endl;
    ll best = -1;
    rep(i, 0, all_N) {
      rep(j, i + 1, all_N) {
        rep(k, j + 1, all_N) {
          vector<Pos> _moves;
          _moves.push_back(all_moves[i]);
          if (2 <= _m_N) _moves.push_back(all_moves[j]);
          if (3 <= _m_N) _moves.push_back(all_moves[k]);

          ll mi = INFF;
          rep(l, 0, (ll)exits.size()) {
            rep(m, l + 1, (ll)exits.size()) {
              ll sum = 0;
              for (auto _move : _moves) {
                Pos to1((Y[exits[l]] + _move.y + L) % L,
                        (X[exits[l]] + _move.x + L) % L);
                Pos to2((Y[exits[m]] + _move.y + L) % L,
                        (X[exits[m]] + _move.x + L) % L);
...
その後, 被っているwholeに対し_m_N <= 3個を調べフローをした.
*/
