...
struct Solver_200_P {
  ...
  void decide_0_pos(vector<Pos> &_moves, vvll &P) {
    ...
    rep(i, 0, L) {
      rep(j, 0, L) {
        ...
        ll sum2 = 0;
        rep(dir, 0, 4) {
          Pos to((i + dy[dir] + L) % L, (j + dx[dir] + L) % L);
          sum1 += pow(P[to.y][to.x], 2);
          sum2 += pow(1000 - P[to.y][to.x], 2);
        }
        if (chmax(best1, sum1)) pos_min = Pos(i, j);
        if (chmax(best2, sum2)) pos_max = Pos(i, j);
      }
    }
    if(best1 == -1 || best2 == -1)cerr << "out" << endl;
    P[pos_min.y][pos_min.x] = 0;
    P[pos_max.y][pos_max.x] = 1000;
  }
  ...
  void create_P(vvll &P) {
    ...
    sa_place_time = sa_place_time - sa_init_time;
  ...
  void new_moves(vvll &P, vector<Pos> &ret_moves, vll &wholes, vll &exits,
    ...
    rep(num, 0, _N) {
      ...
      rep(i, 0, _N) {
        rep(j, i + 1, _N) {
          ll sum = 0;
          for (auto _move : ret_moves) {
            Pos to1((Y[exits[i]] + _move.y + L) % L,
                    (X[exits[i]] + _move.x + L) % L);
            Pos to2((Y[exits[j]] + _move.y + L) % L,
                    (X[exits[j]] + _move.x + L) % L);
            ll val1 = P[to1.y][to1.x];
            ll val2 = P[to2.y][to2.x];
            sum += pow(val1 - val2, 2);
          }
          if (ret_moves.size() == 0) {
            sum =
                abs(Y[exits[i]] - Y[exits[j]]) + abs(X[exits[i]] - X[exits[j]]);
          }
          if (chmin(mi, sum)) best_pair = {i, j};
        }
      }
      Pos cu1(Y[exits[best_pair.first]], X[exits[best_pair.first]]);
      Pos cu2(Y[exits[best_pair.second]], X[exits[best_pair.second]]);
      Pos min_move1 = move(cu1, pos_min);
      Pos min_move2 = move(cu2, pos_min);
      Pos max_move1 = move(cu1, pos_max);
      Pos max_move2 = move(cu2, pos_max);
      ll ma = -1;
      Pos _move;
      ll val1 =
          P[(Y[cu2.y] + min_move1.y + L) % L][(X[cu2.x] + min_move1.x + L) % L];
      ll val2 =
          P[(Y[cu1.y] + min_move2.y + L) % L][(X[cu1.x] + min_move2.x + L) % L];
      ll val3 =
          P[(Y[cu2.y] + max_move1.y + L) % L][(X[cu2.x] + max_move1.x + L) % L];
      ll val4 =
          P[(Y[cu1.y] + max_move2.y + L) % L][(X[cu1.x] + max_move2.x + L) % L];
      if (chmax(ma, abs(val1 - 1000))) _move = min_move1;
      if (chmax(ma, abs(val2 - 1000))) _move = min_move2;
      if (chmax(ma, abs(val3 - 0))) _move = max_move1;
      if (chmax(ma, abs(val4 - 0))) _move = max_move2;
      ...
      rep(j, 0, (ll)wholes.size()) {
        rep(k, 0, _m_N) {
          ...
          rep(num, 0, _Bio_N) {
            ...
            sum += m;
            if (m == 1000) sum += 317 * S / 400;
            if (m == 0) sum += -317 * S / 400;
          }
...
//         if (A[wholes[j]] != E[wholes[j]]) {
//           cerr << "(" << Y[A[wholes[j]]] << ", " << X[A[wholes[j]]] << ") ";
//           cerr << "(" << Y[E[wholes[j]]] << ", " << X[E[wholes[j]]] << ") ";
//         }
//         cerr << endl;
//       }
//       cerr << endl;
...
/*
_m_Nのうち一つは0にする.
0にするマスは周囲との差が一番大きいところにした.
また, 1000になるマスも追加しどちらの方が効果的か調べる
*/
