...
ll optimize_K(ll N, double p1, double p2) {
  ...
  double ma = 0.0;
  int K = 0;
  rep(i, 1, N + 1) if (chmax(ma, min(c_p1[i - 1], 1 - c_p2[i - 1]))) K = i;
  return K;
}
...
struct Solver_200_P {
  ...
  Solver_200_P() {
    ...
    Bio_N = 10000 / (m_N * N);
    Bio_K = optimize_K(Bio_N, 0.5, 0.98);
...
// 概要
/*
p >= 200 のとき, 数か所 Max_P, その他 Min_Pにする. (統計時は p >= 200を外してp<200でも効果的か調べた)
x (N <= 2^x を満たす最小のx) 個方向を選択し, 出口m_i, m_j
に対してそれぞれのx方向先が違うと区別できる.
以上より, local4の, N * N -> N *log(N) になり,
x方向先の区別が1~4回から14~27回(10^4/100/7=14~10^4/60/6=27)
使えてよさそう.
*/
