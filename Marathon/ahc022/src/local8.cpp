...
struct SA_place {
  ...
  bool modify(vvll &_dir_bit, vvll &_P) {
    map<ll, ll> ma;
    rep(i, 0, N) ma[_dir_bit[Y[i]][X[i]]] = i;
    ...
    ll bit_idx = rand() % m_N;
    ll idx = rand() % N;
    ll _bit = _dir_bit[Y[idx]][X[idx]];
    Pos to((Y[idx] + moves[bit_idx].y + L) % L,
           (X[idx] + moves[bit_idx].x + L) % L);
    ll _sum = 0;
    while (1) {
      if (over[to.y][to.x] == 1) break;
      _sum++;
      if (_sum == (ll)100) break;
      bit_idx = rand() % m_N;
      idx = rand() % N;
      _bit = _dir_bit[Y[idx]][X[idx]];
      to = Pos((Y[idx] + moves[bit_idx].y + L) % L,
               (X[idx] + moves[bit_idx].x + L) % L);
    }
    if (_sum == (ll)100) return false;

    ll idx2 = ma.count(_bit ^ (1LL << (bit_idx)))
                  ? ma[_bit ^ (1LL << (bit_idx))]
                  : -1;
    ll _bit2 = -1;
    Pos to2;
    ...
    if (idx2 != -1) {
      _bit2 = (_bit ^ (1LL << (bit_idx)));
      to2 = Pos((Y[idx2] + moves[bit_idx].y + L) % L,
                (X[idx2] + moves[bit_idx].x + L) % L);
      if (over[to2.y][to2.x] != 1) {
        return false;
      }
    }
    ...
    if (_P[to.y][to.x] == Max_P) _dir_bit[Y[idx]][X[idx]] += (1LL << (bit_idx));
    else if (_P[to.y][to.x] == Min_P)
      _dir_bit[Y[idx]][X[idx]] -= (1LL << (bit_idx));
    if (idx2 != -1) {
      if (_P[to2.y][to2.x] == Min_P) _P[to2.y][to2.x] = Max_P;
      else if (_P[to2.y][to2.x] == Max_P) _P[to2.y][to2.x] = Min_P;
      if (_P[to2.y][to2.x] == Max_P)
        _dir_bit[Y[idx2]][X[idx2]] += (1LL << (bit_idx));
      else if (_P[to2.y][to2.x] == Min_P)
        _dir_bit[Y[idx2]][X[idx2]] -= (1LL << (bit_idx));
    }
...
struct Solver_200_P {
  ...
  const double sa_eliminate_time = 0.5;
  const double sa_place_time = 2.5;
  const double create_P_time = 3.5;
  ll Success_num = 1;
  ll Bio_N;
  ll Bio_K;
  ...
  Solver_200_P() {
    ...
    // Bio_K = optimize_K(
    //     Bio_N, 0.5,
    //     0.5 * (1.0 + std::erf((Max_P - Min_P) / S / std::sqrt(2.0))), p);
    // cerr << Bio_N << " " << Bio_K << " " << m_N << " " << at << " "
    //      << 0.5 * (1.0 + std::erf((Max_P - Min_P) / S / std::sqrt(2.0))) << "
    //      "
    //      << Th << endl;

    // Bio_K = optimize_K(Bio_N, 0.5, 0.98, p);
