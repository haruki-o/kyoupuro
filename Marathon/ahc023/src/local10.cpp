...
struct SA_plant {
  ...
  ll optimize_period_num(ll l, ll r, ll num, vvvll &works, ll depth,
                         vvp &period_split_num, vvll &period_first_split,
                         vp &range) {
    ll ret = 0;
    ...
    if (depth == 1) {
      ...
      ll _num = 2;
        ll first_num = _num;
        ll second_num = num - _num;
        ll dfs_l = -1, dfs_r = T + 1;
        ...
        rep(i, l, r + 1) {
          rep(j, i + 1, r + 1) {
            ...
            for (auto [_l, _r] : range) {
              ...
              if (second_num < _sum) break;
              ll ma = second_num - _sum;
              ma = min(ma, (ll)works[toi][toj].size());
              second_sum += (toj - toi + 1) * ma;
              _sum += ma;
              dfs_l = max(toi, dfs_l);
              dfs_r = min(toj, dfs_r);
            }
        ...
        ll first_sum = 0;
        ll best_mid = 0;
        rep(mid, dfs_l + 1, dfs_r) {
          ll sum = 0;
          sum +=
              optimize_period_num(dfs_l, mid, first_num, works, 0,
                                  period_split_num, period_first_split, range);
          sum +=
              optimize_period_num(mid + 1, dfs_r, first_num, works, 0,
                                  period_split_num, period_first_split, range);
          if (chmax(first_sum, sum)) best_mid = mid + 1;
        }

        if (chmax(ret, first_sum + second_sum)) {
          period_split_num[l][r] = {first_sum, second_sum};
          period_first_split[l][r] = best_mid;
        }
      // }
    }
    if (depth == 0) {
  ...
  void period_ans_push(ll idx, ll l, ll r, ll num, vvvll &works, vvw &plans) {
    vp range;
  ...
  void plant_plan(ll idx, vvvll &works, vvw &plans, vvll &period) {
    ...
    vvp period_split_num(T + 1, vp(T + 1));
    vvll period_first_split(T + 1, vll(T + 1, 0));
    rep(i, 1, T + 1) {
      rep(j, i + 1, T + 1) {
        // cerr << i << " " << j << endl;
        period_num[i][j] =
            optimize_period_num(i, j, side[idx].size(), works, 1,
                                period_split_num, period_first_split, range);
      }
    }
    ...
    rep(i, 0, (ll)period[idx].size() - 1) {
      ...
      // first[0]
      period_ans_push(idx, l, period_first_split[l][r] - 1,
                      period_split_num[l][r].first, works, plans);
      // first[1]
      period_ans_push(idx, period_first_split[l][r], r,
                      period_split_num[l][r].first, works, plans);
      // second
      period_ans_push(idx, l, r, period_split_num[l][r].second, works, plans);
    }
  }
