...
struct SA_side {
  ...
  bool modify(vvll &side, ll flag) {
    ll N = side.size();
    vp num(N);
    rep(i, 0, N) num[i] = {side[i].size(), i};
    gSort(num);
    ll idx = num[rand() % min((ll)5, N)].second;
    if (flag != 0) idx = num[rand() % min(flag, N)].second;
  ...
  void solve(Graph &G, vll &road, vvll &side, ll flag) {
...
struct Solver {
  ...
  void solve(vector<Work> &ans) {
    ...
    ll best_p;
    while (1) {
      if (1.8 < ela_times(start_time)) break;
      stat_loop++;
    ...
    while (1) {
      ...
      Graph g;
      vll road;
      construct_road(g, road, best_p);

      vvll side;
      SA_side sa_s(0.05);
      sa_s.solve(g, road, side, para);
