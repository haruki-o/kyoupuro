...
bool operator<(const Work &w1, const Work &w2) {
  return (w1.s < w2.s || (w1.s == w2.s && D[w1.k] < D[w2.k]) ||
          (w1.s == w2.s && D[w1.k] == D[w2.k] && S[w1.k] > S[w2.k]));
}
...
struct Stat {
  ...
  void side_num_map(vll &road, vvll &side) {
    ll N = side.size();
    vvll num(H, vll(W, -10));
    for (ll cu : road)
      num[cu / W][cu % W] = 0;
    rep(i, 0, N) num[side[i][0] / W][side[i][0] % W] = side[i].size();
    rep(i, 0, H) {
      rep(j, 0, W) cout << num[i][j] << " ";
      cout << endl;
    }
  }
...
struct SA_plant {
  ...
  void period_dp_dfs(ll l, ll r, ll num, ll depth, vvvll &period_dp,
                     vvll &period_num, vvvp &period_record) {
    if (depth == 2) return;
    if (period_dp[l][r][num] != -1) return;
    if (num <= 0 || r <= l) {
      period_dp[l][r][0] = 0;
      return;
    }
    ...
    period_dp[l][r][num] = (r - l + 1) * min(num, period_num[l][r]);
    period_record[l][r][num] = {-1, num};

    rep(mid, l + 1, r) {
      rep(c_num, 1, num + 1) {
        period_dp_dfs(l, mid, c_num, depth + 1, period_dp, period_num,
                      period_record);
        period_dp_dfs(mid + 1, r, c_num, depth + 1, period_dp, period_num,
                      period_record);

        ll p_num = num - c_num;
        ll sum = (r - l + 1) * min(p_num, period_num[l][r]);
        sum += period_dp[l][mid][c_num];
        sum += period_dp[mid + 1][r][c_num];
        if (chmax(period_dp[l][r][num], sum)) {
          period_record[l][r][num] = {mid, p_num};
        }
      }
    }
  }

  void period_ans_dfs(ll idx, ll l, ll r, ll num, ll depth, vvvll &period_dp,
                      vvll &period_num, vvvp &period_record, vvw &plans,
                      vvvll &works) {
    // cout << "cu : " << l << " " << r << " " << num;
    // cout << endl;
    // cout << "record: " << period_record[l][r][num].first;
    // cout << " " << period_record[l][r][num].second << endl;
    // cout << endl;

    // 子供
    if (period_record[l][r][num].first == -1) {
      auto ite = works[l][r].begin();
      rep(i, 0, num) {
        if (ite == works[l][r].end()) break;
        plans[idx].push_back(Work(*ite, side[idx][i] / W, side[idx][i] % W, l));
        ite = works[l][r].erase(ite);
      }
      return;
    }

    // dfs
    ll mid = period_record[l][r][num].first;
    ll p_num = period_record[l][r][num].second;
    ll c_num = num - p_num;
    period_ans_dfs(idx, l, mid, c_num, depth + 1, period_dp, period_num,
                   period_record, plans, works);
    period_ans_dfs(idx, mid + 1, r, c_num, depth + 1, period_dp, period_num,
                   period_record, plans, works);
  ...
  void plant_plan(ll idx, vvvll &works, vvw &plans, vvll &period) {
    ...
    rep(i, 1, T + 1) { rep(j, 1, T + 1) period_num[i][j] = works[i][j].size(); }

    vvll period_pre_num(T + 1, vll(T + 1, 0));
    // peroid_pre_num
    rep(i, 1, T + 1) {
      rep(j, i + 1, T + 1) {
        ll _sum = 0;
        for (auto [l, r] : range) {
          ...
          period_pre_num[i][j] += (toj - toi + 1) * ma;
          _sum += ma;
        }
      }
    }
    ...
    // period_dp
    vvvll period_dp(T + 1, vvll(T + 1, vll(side[idx].size() + 1, -1)));
    vvvp period_record(T + 1, vvp(T + 1, vp(side[idx].size() + 1, {-1, -1})));

    rep(i, 1, T + 1) {
      rep(j, i + 1, T + 1) {
        if (30 < j - i) continue;
        rep(num, 0, (ll)side[idx].size() + 1) {
          if (period_dp[i][j][num] == -1)
            period_dp_dfs(i, j, num, 0, period_dp, period_num, period_record);
        }
      }
    }
    ...
    rep(i, 2, T + 1) {
      ll si = side[idx].size();
      if (dp[i] < period_dp[1][i][si]) {
        dp[i] = period_dp[1][i][si];
        pre[i] = {0, 0};
      }
      rep(j, 1, i) {
        if (dp[i] < dp[j] + period_dp[j + 1][i][si]) {
          dp[i] = dp[j] + period_dp[j + 1][i][si];
          pre[i] = {j, 0};
        }
      }
      if (dp[i] < period_pre_num[1][i]) {
        dp[i] = period_pre_num[1][i];
        pre[i] = {0, 1};
      }
      rep(j, 1, i) {
        if (dp[i] < dp[j] + period_pre_num[j + 1][i]) {
          dp[i] = dp[j] + period_pre_num[j + 1][i];
          pre[i] = {j, 1};
        }
      }
    }
    ...
    auto cu = pre[T];
    vll pre_type;
    period[idx].push_back(T + 1);
    while (cu.first != -1) {
      // cout << cu.first + 1<< " " << cu.second << endl;
      period[idx].push_back(cu.first + 1);
      pre_type.push_back(cu.second);
      cu = pre[cu.first];
    }
    reverse(period[idx].begin(), period[idx].end());
    reverse(pre_type.begin(), pre_type.end());

    // period_ans_dfs(), plans
    rep(i, 0, (ll)period[idx].size() - 1) {
      ...
      if (type == 0) {
        ll sum = 0;
        period_ans_dfs(idx, l, r, side[idx].size(), 0, period_dp, period_num,
                       period_record, plans, works);
      }
      ...
      if (type == 1) {
        ...
        ll num = 0;
        rep(j, 0, (ll)at.size()) {
          at[j].i = side[idx][num] / W;
          at[j].j = side[idx][num] % W;
          plans[idx].push_back(at[j]);
          num++;
          if (j != (ll)at.size() - 1) {
            if (at[j].s != at[j + 1].s) num = 0;
          }
        }
      }
    }
  ...
  void optimize_side(ll c, vll &g, vvw &plans, vp decide_ok, vll &used,
                     vvvll &works) {
    // cout << "c : " << c / W << " " << c % W << endl;
    ...
    rep(dir, 0, 4) {
      ...
      if (g[to] == 2) {
        rep(i, 0, N) {
          if (side[i][0] == to) {
            used[i] = 1;
            side_size.push_back(side[i].size());
          }
        }
      }
    }
    ...
    // side_idx : side_size.size()は3以下
    vvll side_idx(3);
    rep(i, 0, (ll)side_size.size()) {
      ll cu_size = side_size[i];
      rep(j, 0, N) {
        if (cu_size == side[j].size() && used[j] == 0) {
          side_idx[i].push_back(j);
        }
      }
    }
    if (side_idx[0].size() == 0) side_idx[0].push_back(-1);
    if (side_idx[1].size() == 0) side_idx[1].push_back(-1);
    if (side_idx[2].size() == 0) side_idx[2].push_back(-1);

    // candi
    vvll candi;
    for (ll i0 : side_idx[0]) {
      for (ll i1 : side_idx[1]) {
        for (ll i2 : side_idx[2]) {
          vll at = {i0, i1, i2};
          if (i0 == i1 || i0 == i2 || (i1 == i2 && i1 != -1)) continue;
          candi.push_back(at);
        }
      }
    }
    ...
    ll idx = -1;
    ll best_score = -1;
    rep(i, 0, (ll)candi.size()) {
      vp ok;
      ok = decide_ok;
      rep(j, 0, 3) {
        if (candi[i][j] == -1) break;
        vp _ok;
        for (auto cu : plans[candi[i][j]]) {
          if (cu.i * W + cu.j == side[candi[i][j]][0]) {
    ...
    rep(dir, 0, 4) {
      ...
      if (g[to] == 2) {
        rep(i, 0, N) {
          if (side[i][0] == to) from_idx.push_back(i);
        }
      }
    }

    // plansのswap(candi[idx][], from_idx[])

    rep(i, 0, 3) {
      if (candi[idx][i] == -1) break;
      ll idx1 = candi[idx][i];
      ll idx2 = from_idx[i];

      vvw pos_ans(H * W);
      for (auto cu : plans[idx1])
        pos_ans[cu.i * W + cu.j].push_back(cu);
      for (auto cu : plans[idx2])
        pos_ans[cu.i * W + cu.j].push_back(cu);

      // plans[idx2] <- plans[idx1]
      vw _plans2;
      rep(j, 0, side[idx1].size()) {
        rep(k, 0, pos_ans[side[idx1][j]].size()) {
          auto cu = pos_ans[side[idx1][j]][k];
          _plans2.push_back(
              Work(cu.k, side[idx1][j] / W, side[idx1][j] % W, cu.s));
        }
      }
      // plans[idx1] <- plans[idx2]
      vw _plans1;
      rep(j, 0, side[idx2].size()) {
        rep(k, 0, pos_ans[side[idx2][j]].size()) {
          auto cu = pos_ans[side[idx2][j]][k];
          _plans1.push_back(
              Work(cu.k, side[idx2][j] / W, side[idx2][j] % W, cu.s));
        }
      }
  ...
  void dfs(ll c, ll p, vll &g, vvw &plans, vw &ans, vll &seen, vvvll &works,
    ...
    rep(dir, 0, 4) {
      ...
      if (!can_through(c, to)) continue;
      if (seen[to] == 1) continue;
      dfs(to, c, g, plans, ans, seen, works, used);
    }

    // cout << "cu : " << c / W << " " << c % W << endl;
    // road かつ 子
    vp decide_ok;
    decide_ok.push_back({1, T});
    // decide_ok
    rep(dir, 0, 4) {
      ll toi = c / W + dy[dir];
      ll toj = c % W + dx[dir];
    ...
    // optimize_side(c, g, plans, decide_ok, used, works);
  ...
  void fill_road(vvw &plans, vw &ans) {
    ...
    rep(i, 0, K) if (_used[i] == 0) works[S[i]][D[i]].push_back(i);

    vll seen(H * W, 0);
    vll used(N, 0);
    dfs(i0 * W, -1, g, plans, ans, seen, works, used);
  }

  void adjust(vvw &plans) {
    vll used(K, 0);
    vvvll works(T + 1, vvll(T + 1));
    ...
    rep(i, 0, K) if (!used[i]) works[S[i]][D[i]].push_back(i);

    rep(i, 0, N) {
      vll period;
      rep(j, 0, (ll)plans[i].size() - 1) {
        if (plans[i][j].s != plans[i][j + 1].s) {
          period.push_back(plans[i][j].s);
        }
      }
      if (!plans[i].empty()) period.push_back(_ans[i][_ans[i].size() - 1].s);

      ll ma = 0;
      ll best_score = -1;
      ll l = -1, r;
      rep(j, 0, period.size()) {
        rep(k, j + 1, period.size()) {
          ll sum = works[period[j]][period[k] - 1].size();
          sum = min(sum, (ll)side[i].size());
          ll score = 0;
          for (auto cu : plans[i]) {
            rep(u, side[i].size() - sum, side[i].size()) {
              if (cu.i * W + cu.j == side[i][u]) {
                if (period[j] <= cu.s && D[cu.k] < period[k]) {
                  score += S[cu.k] - period[j];
                  score += period[k] - D[cu.k];
                }
              }
            }
          }
      ...
      // ずらす
      rep(j, 0, (ll)plans[i].size() - ma) {
        auto cu = plans[i][j];
        if (cu.s < l || r < cu.s) continue;
        if (plans[i][j].s == plans[i][j + ma].s) {
          plans[i][j].k = plans[i][j + ma].k;
        }
      }

      // 後半ma個消す
      auto ite = plans[i].begin();
      while (ite != plans[i].end()) {
        if ((*ite).s < l || r < (*ite).s) {
          ite++;
        } else {
          int f = 0;
          rep(j, side[i].size() - ma, side[i].size()) {
            if ((*ite).i * W + (*ite).j == side[i][j]) {
              ite = plans[i].erase(ite);
              // cout << "x";
              f = 1;
              break;
            }
      ...
      ll push_num = 0;
      auto ite1 = works[l][r].begin();
      while (ite1 != works[l][r].end()) {
        ll _at = side[i][side[i].size() - 1 - push_num];
        plans[i].push_back(Work(*ite1, _at / W, _at % W, l));
        ite1 = works[l][r].erase(ite1);
        push_num++;
        if (push_num == ma) break;
      }
      Sort(plans[i]);

      // cout << "a aft" << endl;
      // for (auto cu : plans[i]) {
      //   cout << "a " << cu.k << " " << S[cu.k] << " " << D[cu.k] << " " <<
      //   cu.s; cout << " {" << cu.i << ", " << cu.j << "}" << endl;
      // }
    }
  }
...
struct Solver {
  ...
  void solve(vector<Work> &ans) {
    ...
    // ll esti = 0;
    // rep(i, 0, H - 1) rep(j, 0, W) esti += h[i][j];
    // rep(i, 0, H) rep(j, 0, W - 1) esti += v[i][j];

    rep(para, 4, 16) {
      Graph g;
      vll road;
      construct_road(g, road, para);
      ...
      ll ma = 0;
      rep(i, 0, (ll)side.size()) chmax(ma, (ll)side[i].size());
      if (ma <= 25) {
        _side = side;
        _road = road;
        best_p = para;
    ...
    SA_plant sa_p(_side, 0.5);
    sa_p.solve(ans);

    stat_time = ela_times(start_time);
    // Stat st;
    // st.side_num_map(_road, _side);
    // st.side_num(_side);
    // st.side_color(_side);
    // st.rest_num(ans);

    cerr << "best_p    : " << best_p << endl;
  }
};
...
// local12.cpp (← local8.cpp)

// 考察
/*
Solve.solve():
  adjuct()を追加. plans[]内で結合できる

めっっちゃ微増(1ケース2,000)ほど
*/
