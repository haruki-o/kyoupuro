...
struct SA_plant {
  ...
  void plant_plan(ll idx, vvvll &works, vvw &plans, vw &ans, ll &r_pos) {
    vp ok;
    for (auto cu : ans) {
      if (r_pos == cu.i * W + cu.j) {
        ok.push_back({cu.s, D[cu.k]});
      }
    }
    ok.push_back({-1, 0});
    ok.push_back({T + 1, T + 1});
    Sort(ok);

    // l_ok, r_ok
    vll l_ok(T + 1, 0), r_ok(T + 1, 0);
    rep(i, 0, ok.size() - 1) {
      rep(j, ok[i].second + 1, ok[i + 1].first + 1) {
        if (1 <= j && j <= T) l_ok[j] = 1;
      }
      rep(i, 0, ok.size() - 1) {
        rep(j, ok[i].second, ok[i + 1].first) {
          if (1 <= j && j <= T) r_ok[j] = 1;
        }
      }
    }
    ...
    rep(i, 1, T + 1) {
      rep(j, i + 1, T + 1) {
        ...
        for (auto [l, r] : range) {
          ...
          if(l_ok[toi] == 0 || r_ok[toj] == 0)continue;
    ...
    ll cu = T;
    while (cu != 0) {
      period.push_back(cu + 1);
      cu = pre[cu];
    }
    period.push_back(1);
    reverse(period.begin(), period.end());

    // rep(i,0,(ll)period.size() - 1){
  ...
  void init(vw &ans, vvw &plans) {
    ...
    while (!qu.empty()) {
      ...
      rep(dir, 0, 4) {
        ...
        if (g[to] == 2) {
          rep(i, 0, N) {
            if (side[i][0] == to) at[i] = {dist[cu], i, cu};
          }
        }
      }
    }
    ...
    rep(_i, 0, N) {
      auto [num, i, road_pos] = at[_i];
      plant_plan(i, works, plans, ans, road_pos);
    }
  ...
  void optimize_road(ll c, vw &ans, vvvll &works, pll &ok, ll upper) {
    vvll period_num(T + 1, vll(T + 1, 0));
    ...
    rep(i, 1, T + 1) {
      rep(j, i + 1, T + 1) {
        ...
        for (auto [l, r] : range) {
          ...
          ll toj = j - r;
          if (toi >= toj) continue;
          if (works[toi][toj].size() == 0) continue;
          period_num[i][j] += (toj - toi + 1);
          break;
        }
      }
    }
    ...
    vll pre(T + 1, 0);
    // dp
    rep(i, ok.first + 1, T + 1) {
      if (dp[i] < period_num[ok.first][i] && i - ok.first + 1 <= upper) {
        dp[i] = period_num[ok.first][i];
        pre[i] = ok.first - 1;
      }
      rep(j, ok.first, i) {
        if (dp[i] < dp[j] + period_num[j + 1][i] && i - j <= upper) {
          dp[i] = dp[j] + period_num[j + 1][i];
          pre[i] = j;
        }
      }
    ...
    rep(i, 0, (ll)period.size() - 1) {
      ...
      for (auto [_l, _r] : range) {
        ...
        if (works[toi][toj].size() == 0) continue;
        auto ite = works[toi][toj].begin();
        ans.push_back(Work(*ite, c / W, c % W, toi));
        ite = works[toi][toj].erase(ite);
        break;
      }
    }
  ...
  void dfs(ll c, ll p, vll &g, vw &ans, vll &seen, vvvll &works, ll upper) {
    seen[c] = 1;
    rep(dir, 0, 4) {
      ll toi = c / W + dy[dir];
      ll toj = c % W + dx[dir];
      ...
      if (!can_through(c, to)) continue;
      if (seen[to] == 1) continue;
      dfs(to, c, g, ans, seen, works, upper);
    }
    ...
    for (auto cu : ok)
      optimize_road(c, ans, works, cu, upper);
  }

  void fill_road(vw &ans, ll upper) {
    vll g(H * W, 1);
    rep(idx, 0, N) {
      for (auto cu : side[idx])
        g[cu] = 0;
    ...
    vll seen(H * W, 0);
    dfs(i0 * W, -1, g, ans, seen, works, upper);
  }
...
// local9.cpp (← local7.cpp)

// 考察
/*
Solve.solve():
  road()を埋めて, sid()を埋めました.
失敗
*/
