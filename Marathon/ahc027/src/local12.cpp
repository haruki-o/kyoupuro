...
struct Solver {
  ...
  void solve(string &ans) {
    ...
    rep(i, 0, order.size()) {
      if (order[i].second == 1)
        used[order[i].first.first][order[i].first.second] = 1;
    }
    rep(i, 0, N) {
      rep(j, 0, N) cout << used[i][j] << " ";
      cout << endl;
    }
...
// local12.cpp <- local9.cpp
// free_route()を変更しました. 没