...
struct Solver {
  ...
  void decide_op_s(string &op_s, vvll &g, vpp &order, map<pll, string> &store0,
    ...
    rep(i, 1, order.size()) {
      ...
      if (order[i].second == 0) {
        if (store0.count({cu, to})) op_s += store0[{cu, to}];
        else {
          string _op_s =
              route_free(order[i - 1].first, order[i].first, op_s, g);
          op_s += _op_s;
          store0[{cu, to}] = _op_s;
        }
      } else {
        if (store1.count({cu, to})) op_s += store1[{cu, to}];
        else {
  ...
  void calc_leaf_eval(vector<pair<ll, pll>> &leaf_eval, vvll &g) {
    vp at;
    rep(y, 0, N) {
      rep(x, 0, N) {
        ll sum = 0;
        ...
        at.push_back({sum, y * N + x});
      }
    }
    gSort(at);
    for (auto cu : at) {
      leaf_eval.push_back({cu.first, {cu.second / N, cu.second % N}});
    }
  }

  // 2,5,4,3 -> 2,2,5,4,3 -> 2,3,2,5,4,3
  void modify(vpp &order, vector<pair<ll, pll>> &leaf_eval) {
    ll si = order.size();
    pll add_cor;
    ll sum = 0;
    rep(i, 0, leaf_eval.size()) { sum += leaf_eval[i].first; }
    ll ran = rand() % sum;
    sum = 0;
    rep(i, 0, leaf_eval.size() - 1) {
      if (sum <= ran && ran <= sum + leaf_eval[i + 1].first) {
        add_cor = leaf_eval[i].second;
      }
      sum += leaf_eval[i].first;
    }

    ll ins_idx = rand() % (si - 1);
    if (order[ins_idx + 1].second == 0 && rand() % 2 == 0) {
      order.insert(order.begin() + ins_idx + 1, {add_cor, (ll)0});
    } else {
  ...
  void solve(string &ans) {
    ...
    ll best_score = INFF;
    rep(i, 0, 10) {
      map<pll, string> _store0, _store1;
      vpp _order;
      vvll _g(N * N);
      _order.push_back({{0, 0}, 1});
      construct_g(_g, _order);
      _order.push_back({{0, 0}, 1});
      string _op_s = "";
      decide_op_s(_op_s, _g, _order, _store0, _store1);
    ...
    vector<pair<ll, pll>> leaf_eval;
    calc_leaf_eval(leaf_eval, g);
    // rep(i,0,leaf_eval.size()){
    //   cerr << leaf_eval[i].first << " {" << leaf_eval[i].second.first << ","
    //   << leaf_eval[i].second.second << endl;
    // }
...
// local13.cpp <- local10.cpp
// 焼きなましのmodify()で周辺のdが高い所に行くようにしました
// あまり変わらない