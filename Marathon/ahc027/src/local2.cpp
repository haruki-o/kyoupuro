...
struct Solver {
  ...
  void construct_dfs(ll c, ll p, vvll &g, vll &visited, vp &order) {
    visited[c] = 1;
    ...
    if (c != 0 && g[c].size() == 1) order.push_back({cu_y, cu_x});
  }

  void construct_uf(vvll &g, vp &order) {
    vp ava;
    rep(i, 0, N) {
      rep(j, 0, N) {
        rep(dir, 2, 4) {
    ...
    rep(i, 1, N * N) {
      if (g[i].size() == 1) order.push_back({i / N, i % N});
    }
  }
  void construct_g(vvll &g, vp &order) {
    vll visited(N * N, 0);
    // construct_uf(g, order);
  ...
  void decide_op_s(string &op_s, vvll &g, vp &order) {
    rep(i, 1, order.size()) { op_s += route(order[i - 1], order[i], g); }
  }

  void modify(vp &order) {
    ll si = order.size();
    ll add_idx = rand() % (si - 2);
    add_idx++;
    ll ins_idx = rand() % (si - 1);
    ins_idx++;
    order.insert(order.begin() + ins_idx, order[add_idx]);
  }
  ...
  void solve(string &ans) {
    ...
    while (1) {
      if (1 < ela_times(start_time)) break;
      stat_loop++;
