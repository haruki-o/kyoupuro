...
struct Solver {
  ...
  ll edit_a(pll cu, vvll &a, string &op_s) {
    ...
    rep(i, 0, N) {
      rep(j, 0, N) {
        if (g[i][j].size() == 0) {
          ll l = a[i][j];
          ll r = l + d[i][j] * op_s.size();
          ll num = op_s.size();
          ret += (l + r) * num / 2 / op_s.size();
  ...
  string route_add(pll &cu, vvll &a, ll type) {
    // {ava, dir}
    vvp dp(N, vp(N, {0, -2}));
    // {ava, sum, num, dir, cor}
    priority_queue<tuple<ll, ll, ll, ll, pll>,
    ...
    rep(i, 0, N) {
      rep(j, 0, N) {
        if (chmax(best, dp[i][j].first)) to = {i, j};
      }
    }
    ...
    if (type == 1 && 10 < ret.size()) {
      ret = ret.substr(0, 10);
      rep(i, 0, ret.size()) {
        if (ret[i] == 'U') cu.first -= 1;
        if (ret[i] == 'D') cu.first += 1;
        if (ret[i] == 'L') cu.second -= 1;
        if (ret[i] == 'R') cu.second += 1;
      }
    } else cu = store_to;
  ...
  void solve(string &ans) {
    ...
    while (1) {
      ...
      pll _cu_cor = cu_cor;
      string _op_s = route_add(_cu_cor, a, type);
      if (1e5 - 500 < _op_s.size() + op_s.size()) break;
      vvll _a(N, vll(N));
      _a = a;
      ll score = edit_a(cu_cor, _a, _op_s);
      if (mid < score && type == 0) {
        type++;
        cerr << mid << " " << score << " " << _op_s.size() << " ";
        continue;
      }
      if (type == 1) cerr << score << " " << _op_s.size() << endl;

      type = 0;
      if (mid == INF) mid = score * 1.1;
      else
        mid = (mid * op_s.size() + score * _op_s.size()) /
              (op_s.size() + _op_s.size());
      // cerr << op_s.size() << " " << score << endl;
      op_s += _op_s;
      cu_cor = _cu_cor;
      a = _a;
    }
