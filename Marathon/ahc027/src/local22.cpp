...
struct Solver {
  ...
  string route_add(pll &cu, vvll &a) {
    ...
    vvT dp(N, vT(N, {0, 1, -2}));
    dp[cu.first][cu.second] = {a[cu.first][cu.second], 1, -1};
    visited[cu.first][cu.second] = 1;
    ...
    while (1) {
      ...
      rep(dir, 0, 4) {
        ...
        if (is_wall(cu, dir)) continue;
        if (visited[to_y][to_x]) continue;
        vvll at_visited(N, vll(N, 0));
        at_visited = visited;
        at_visited[to_y][to_x] = 1;
        ll score = -1;
        dfs({to_y, to_x}, {-1, -1}, a[to_y][to_x], score, at_visited, visited, a, 1);
      ...
      visited[best_cor.first][best_cor.second] = 1;
      ll mid = (get<0>(dp[cu.first][cu.second]) +
                a[best_cor.first][best_cor.second]) /
               (get<1>(dp[cu.first][cu.second]) + 1);
      dp[best_cor.first][best_cor.second] = {
          mid, get<1>(dp[cu.first][cu.second]) + 1, best_dir};
      cu = best_cor;
    }
    ...
    string ret = "";
    while (get<2>(dp[to.first][to.second]) != -1) {
      ret += move_c[get<2>(dp[to.first][to.second])];
      to = to_dir(to.first, to.second,
                  (get<2>(dp[to.first][to.second]) + 2) % 4);
    }
    reverse(ret.begin(), ret.end());
    return ret;
  }
  ...
  void solve(string &ans) {
    ...
    while (1) {
      ...
      if (1e5 - 500 < _op_s.size() + op_s.size()) continue;
