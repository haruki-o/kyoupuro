...
struct Solver {
  ...
  void dfs(pll c, pll p, ll score, ll &best_score, vvll &at_visited,
           vvll &visited, vvll &a, ll length) {
    if (length == 3) {
    ...
    rep(dir, 0, 4) {
      auto [to_y, to_x] = to_dir(c.first, c.second, dir);
      if (is_out(to_y, to_x)) continue;
      if (is_wall(c, dir)) continue;
      if (to_y == p.first && to_x == p.second) continue;
      if (at_visited[to_y][to_x]) continue;
      if (visited[to_y][to_x]) continue;
      at_visited[to_y][to_x] = 1;
      dfs({to_y, to_x}, c, score + a[to_y][to_x], best_score, at_visited,
          visited, a, length + 1);
      at_visited[to_y][to_x] = 0;
    }
    score -= a[c.first][c.second];
  }

  string route_add(pll &cu, vvll &a, ll &ava, ll mid_ava, vp &dirtys) {
    // {ava, dir}
    vvp dp(N, vp(N, {0, -2}));
    // {ava, sum, num, dir, cor}
    priority_queue<tuple<ll, ll, ll, ll, pll>,
    ...
    ava += dp[to.first][to.second].first;

    if (dp[to.first][to.second].first * 18 /10< mid_ava) {
      rep(i, 0, N) dp[i].assign(N, {0, -2});
      queue<pll> qu;
      qu.push(cu);
      dp[cu.first][cu.second] = {0, -1};
      while (!qu.empty()) {
        auto [cu_y, cu_x] = qu.front();
        qu.pop();
        rep(dir, 0, 4) {
          ...
          if (is_wall({cu_y, cu_x}, dir)) continue;
          if (dp[to_y][to_x].second != -2) continue;
          dp[to_y][to_x] = {dp[cu_y][cu_x].first + 1, dir};
          qu.push({to_y, to_x});
        }
      }
      ll ma = -1;
      for (auto _to : dirtys) {
        ll sum = 0;
        if(_to.first == cu.first && _to.second == cu.second)continue;
        rep(y, -1, 2) {
          rep(x, -1, 2) {
            ll to_y = _to.first + y;
            ll to_x = _to.second + x;
            if (is_out(to_y, to_x)) continue;
            sum += a[to_y][to_x];
          }
        }
        if (chmax(ma, sum)) to = _to;
      }
    }
  ...
  ll calc_mid_ava_test(pll cu_cor, vvll a, string op_s) {
    ll ret = 0;
    auto start_time = system_clock::now();
    ll at_loop_num = 0;
    while (1) {
      if (0.1 < ela_times(start_time)) break;
      at_loop_num++;

      pll _cu_cor;
      _cu_cor = cu_cor;
      vp _;
      string _op_s = route_add(_cu_cor, a, ret, -1, _);
  ...
  void candi_dirty_cor(vp &dirtys) {
    vector<pair<ll, pll>> candi;
    rep(i, 0, N) {
      rep(j, 0, N) {
        ll sum = 0;
        rep(y, -1, 2) {
    ...
    for (auto cu : candi) {
      ll cu_cost = cu.first;
      pll cu_cor = cu.second;
      if (cu_cost < ma * 3 / 4 && 2 <= dirtys.size() ) break;
      int f = 1;
      for (auto to : dirtys) {
        ll dist = abs(cu_cor.first - to.first) + abs(cu_cor.second - to.second);
        if (dist < (ll)N / 2) {
          f = 0;
          break;
        }
      ...
      if (!f) continue;
      dirtys.push_back(cu_cor);
      if (dirtys.size() == 3) break;
    }
  }
 ...
 // 2,5,4,3 -> 2,2,5,4,3 -> 2,3,2,5,4,3
  ...
  void solve(string &ans) {
    ...
    // cerr << ans_calc_score(op_s) << endl;

    ll mid_ava = calc_mid_ava_test(cu_cor, a, op_s);
    // cerr << mid_ava << endl;

    vp dirtys;
    candi_dirty_cor(dirtys);
    // cerr << dirtys.size() << endl;
    // for (auto cu : dirtys)
    //   cerr << "{" << cu.first << " " << cu.second << "}";
    // cerr << endl;
    ...
    while (1) {
      ...
      _cu_cor = cu_cor;
      ll _ = 0;
      string _op_s = route_add(_cu_cor, a, _, mid_ava, dirtys);
      if (1e5 - 500 < _op_s.size() + op_s.size()) break;
      ll score = edit_a(cu_cor, a, _op_s);
      // cerr << op_s.size() <<  " " << _op_s.size() << " " << score << endl;
      op_s += _op_s;
      cu_cor = _cu_cor;
    }
...
// local22.cpp <- local17.cpp
// add_route()をビームサーチ風に変更しました. 没