...
struct Solver {
  ...
  void solve(string &ans) {
    ...
    ll _best_score = ans_calc_score(op_s);

    while (1) {
      if (0.5 < ela_times(start_time)) break;
    ...
    while (1) {
      ...
      rep(i, 0, 3) {
        ...
        rep(j, 0, 4) {
          pll at_cu_cor = _cu_cor;
          string at_op_s = route_add(_cu_cor, _a);
          _op_s += at_op_s;
          _score += edit_a(at_cu_cor, _a, at_op_s) * at_op_s.size();
        }
        if (_op_s.size() == 0) continue;
        _score /= _op_s.size();
...
// local25.cpp <- local24.cpp
// route_addを3個試して採用にしました