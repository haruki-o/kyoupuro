...
struct Solver {
  ...
  void solve(string &ans) {
    ...
    pll cu_cor = {0, 0};
    vvll a(N, vll(N, 0));
    rep(i, 0, N) rep(j, 0, N) a[i][j] = d[i][j] * 10;
    ...
    while (1) {
      ...
      if (1e5 - 1000 < best_op_s.size() + op_s.size()) break;
    ...
    if (ans_calc_score(op_s) == INF) {
      op_s = op_s.substr(0, 1e5 - 1e4);
      pll cu = {0, 0};
      rep(i, 0, op_s.size()) {
        if (op_s[i] == 'U') cu.first -= 1;
        if (op_s[i] == 'D') cu.first += 1;
