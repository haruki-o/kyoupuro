...
struct Solver {
  ...
  void construct_dfs(ll c, ll p, vvll &g, vll &visited, vpp &order, ll at_dir,
                     ll depth) {
    visited[c] = 1;
    ...
    rep(_dir, 0, 4) {
      ...
      g[to].push_back(c);
      g[c].push_back(to);
      if (f) construct_dfs(to, c, g, visited, order, dir, 0);
      else construct_dfs(to, c, g, visited, order, dir, depth + 1);
    }
  ...
  void construct_g(vvll &g, vpp &order) {
    ...
    construct_dfs(0, -1, g, visited, order, 0, 0);
  }
  ...
  void decide_op_s(string &op_s, vvll &g, vpp &order, map<pll, string> &store) {
    rep(i, 1, order.size()) {
      ...
      } else op_s += route(order[i - 1].first, order[i].first, g);
    }
  }
  ...
  void solve(string &ans) {
    ...
    order.push_back({{0, 0}, 1});
    string op_s = "";
    decide_op_s(op_s, g, order, store);
    ...
    while (1) {
      ...
      string _op_s = "";
      decide_op_s(_op_s, g, _order, store);
