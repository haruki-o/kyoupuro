...
struct Solver {
  ...
  ll select_next_card(const vector<Card> &next_cards, ll money, ll turn) {
    ll next_card_idx = 0;
    vll ava;
    string msg = "";
    rep(i, 0, next_cards.size()) {
      if (next_cards[i].t == CardType::WORK_SINGLE && next_cards[i].p != 0) continue;
      ...
      if (next_cards[i].t == CardType::INVEST && 500 < turn) continue;
  ...
  ll solve() {
    ...
    rep(turn, 0, t) {
      ...
      ll select_card_i = select_next_card(next_cards, money, turn);
