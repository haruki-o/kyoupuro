...
struct Solver {
  ...
  ll select_next_card(const vector<Card> &next_cards, const vector<Card> &cards, vector<Project> &projects, ll money, ll turn, ll invest_level) {
    ...
    rep(i, 1, next_cards.size()) {
      ...
      ll day = 8;
      ll sum = 0;
      rep(j, 0, 5) sum += x_num[j];
      ll E_card_num = (ll)((k - 1) * day * ((double)x_num[4] / sum));
      ll E_bottom_p = (600 / (double)E_card_num + 200) * pow(2, L);
      if (next_cards[i].t != CardType::INVEST) {
        if (E_bottom_p < money) {
          continue;
        }
      }
      // if (next_cards[i].t == CardType::INVEST) cerr << next_cards[i].p << endl;
