  ...
  pll select_action(vector<Card> &cards, vector<Project> &projects, ll money, ll turn) {
    ...
    rep(i, 0, cards.size()) {
      ...
      if (use_card.t == CardType::WORK_SINGLE) {
        ...
        rep(j, 0, projects.size()) {
          ...
          if (950 < turn) {
            ...
            unit = projects[j].v / (double)projects[j].h;
            score = unit * min(projects[j].h, use_card.w);
            ll wd = min(projects[j].h, use_card.w) / pow(2, L);
            ll sum = 0;
            rep(i, 0, 5) sum += x_num[i];
            ll p;
            if (1.0 <= (double)x_num[0] / sum * (k - 1)) {
              p = clamp(wd, 1, 10000) * pow(2, L);
            } else {
              p = clamp(wd, 1, 10000) * pow(2, L);
            }

            if (p < use_card.p) score = -1;
          }
      ...
      if (use_card.t == CardType::CANCEL_SINGLE) {
        ...
        // vで gauss(b,0.5) < b の時 最良が v / h = 1
        if (unit < pow(2, 0)) score = INFF - 1;
        if (chmax(best_score, score)) {
          c = i;
          m = 0;
          rep(j, 0, projects.size()) {
            if (projects[j].v / (double)projects[j].h < projects[m].v / (double)projects[m].h) {
              m = j;
            }
          }
        }
  ...
  ll select_next_card(const vector<Card> &next_cards, const vector<Card> &cards, vector<Project> &projects, ll money, ll turn, ll invest_level) {
    ...
    rep(i, 1, next_cards.size()) {
      ...
      if (next_cards[i].t == CardType::WORK_SINGLE) {
        ...
        ll p;
        if (1.0 <= (double)x_num[0] / sum * (k - 1)) {
          p = clamp(wd - wd / 4, 1, 10000) * pow(2, L);
        } else {
          p = clamp(wd - wd / 5, 1, 10000) * pow(2, L);
        }
      ...
      if (next_cards[i].t == CardType::INVEST) {
        if (invest_level == 20) continue;
        if (next_cards[next_card_idx].t == CardType::INVEST) {
          if (next_cards[next_card_idx].p < next_cards[i].p) {
            continue;
          }
        }
        if (money - next_cards[i].p < 10 * pow(2, L + 1)) continue;
        score = 5;
        // if (600 < turn) {
        //   ll p = 300 * pow(2, L);
        //   if (next_cards[i].p < p) score = 4 + i;
        // } else {
        //   ll p = 400 * pow(2, L);
        //   if (next_cards[i].p < p) score = 4 + i;
        // }
