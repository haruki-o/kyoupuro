...
struct Simulate {
  ...
  ll money, _turn, invest_level;
  vll x;

  Simulate(ll n, ll m, ll k, ll t, vector<Card> cards, vector<Project> projects, ll money, ll _turn, ll invest_level, vll x)
      : n(n), m(m), k(k), t(t), cards(cards), projects(projects), money(money), _turn(_turn), invest_level(invest_level),x(x) {
  }

  ll generateCardType() {
    // 確率分布の総和を計算
    int totalProbability = 0;
    for (int probability : x) {
  ...
  void process(Card use_card, ll use_card_i) {
    cards[use_card_i] = use_card;
    money -= use_card.p;
    _turn++;
  }
  ...
  ll select_next_card(const vector<Card> &next_cards, const vector<Card> &cards, vector<Project> &projects, ll money, ll turn, ll invest_level) {
    ...
    rep(i, 1, next_cards.size()) {
      ...
      if (next_cards[i].t == CardType::WORK_ALL) {
        double score = 0;
        ll wd = next_cards[i].w / pow(2, L);
        ll p = clamp(wd * m - wd * m / 4, 1, 10000) * pow(2, L);
        if (next_cards[i].p < p) score = 1;
  ...
  void solve(double &score) {
    rep(turn, _turn, t) {
      ...
      after_use_action(use_card_i, use_target, cards, projects, money, invest_level);

      vector<Card> next_cards = read_next_cards(invest_level, turn);
...
struct Solver {
  ...
  void registe_x(vector<Card> &next_cards) {
    rep(i, 1, next_cards.size()) {
      if (next_cards[i].t == CardType::WORK_SINGLE) x_num[0]++;
      if (next_cards[i].t == CardType::WORK_ALL) x_num[1]++;
      if (next_cards[i].t == CardType::CANCEL_SINGLE) x_num[2]++;
      if (next_cards[i].t == CardType::CANCEL_ALL) x_num[3]++;
      if (next_cards[i].t == CardType::INVEST) x_num[4]++;
    }
  }

  ll select_next_card_simulate(const vector<Card> &next_cards, ll use_card_i, const vector<Card> &cards, const vector<Project> &projects, ll money, ll turn, ll invest_level) {
    ll next_card_idx = 0;
    ll L = invest_level;
    double best_score = 0;
    rep(i, 0, next_cards.size()) {
      if (money - next_cards[i].p < 0) continue;
      double score = 0;
      Simulate simulate(n, m, k, t, cards, projects, money, turn, invest_level, x_num);
      simulate.process(next_cards[i], use_card_i);
      simulate.solve(score);
  ...
  ll solve() {
    ...
    rep(turn, 0, t) {
      ...
      registe_x(next_cards);

      ll select_card_i;
      if (turn < 900) {
        select_card_i = select_next_card(next_cards, cards, projects, money, turn, invest_level);
      } else {
        select_card_i = select_next_card_simulate(next_cards, use_card_i, cards, projects, money, turn, invest_level);
      }
      cards[use_card_i] = next_cards[select_card_i];
