...
struct PressG {
  ...
  void adjustClusterNum() {
    ...
    ll loop1 = 0, loop2 = 0;
    while (ela_times(startTime) < limitTime) {
      loop1++;
      ll idx1 = rand() % size;
      while (clusterToVertices[idx1].size() <= LB) {
        idx1 = (idx1 + 1) % size;
        if (ela_times(startTime) < limitTime) break;
      }
      ll idx2 = rand() % size;
      pll edge;
      rep(i, 0, size) {
  ...
  // PressGを構築する
  void constructPressG() {
    UnionFind uf(N);
    ll seedNum = N / LB + 10;
    rep(i, 0, seedNum) clusterToVertices[i].push_back(i);
    vll used(N, 0);
    rep(i, 0, seedNum) used[i] = 1;
    while (1) {
      int _flag = 0;
      rep(i, 0, seedNum) {
        int flag = 0;
        for (ll cu : clusterToVertices[i]) {
          for (ll to : g[cu]) {
            if (!used[to]) {
              used[to] = 1;
              clusterToVertices[i].push_back(to);
              flag = 1;
              _flag = 1;
              break;
            }
    ...
    // cerr << "pre composeClusterVerticeNum" << endl;
    // for (auto cu : clusterToVertices) cerr << cu.second.size() << " ";
    // cerr << endl;
    // cerr << clusterToVertices.size() << endl;
    adjustClusterNum();
    // cerr << "after composeClusterVerticeNum" << endl;
    // for (auto cu : clusterToVertices) cerr << cu.second.size() << " ";
    // cerr << endl;
    // cerr << clusterToVertices.size() << endl;
    ...
    G.resize(clusterToVertices.size());
    rep(i, 0, G.size()) {
      rep(j, i + 1, G.size()) {
...
struct Solver {
  ...
  void constructADfs(ll v, vll &A, PressG &pressG, vll &visited) {
    // cerr << v << " ";
    visited[v] = 1;
    pressG.clusterToAIndex[v] = A.size();
    for (auto cu : pressG.clusterToVertices[v]) A.push_back(cu);
    for (ll c : pressG.G[v]) {
      if (visited[c] == 1) continue;
      visited[c] = 1;
      constructADfs(c, A, pressG, visited);
    }
  }

  void initConstructA(vll &A, PressG &pressG) {
    vll visited(pressG.G.size(), 0);
    constructADfs(0, A, pressG, visited);
    ll addNum = LA - A.size();
    rep(i, 0, addNum) A.push_back(0);
    vll seen(N, 0);
    for (ll cu : A) seen[cu] = 1;
  }
  ...
  void solve(vector<Event> &ans, vll &A) {
    ...
    constructTree(tree);
    PressG pressG;
    pressG.constructPressG();

    // vll A;
    initConstructA(A, pressG);
    // cerr << "A[] : ";
    // rep(i, 0, LA) cerr << A[i] << " ";
...
void print_ans(vector<Event> &ans, vll &A) {
...
// void adjustClusterNum() 内の無限ループを直しただけ