...
struct Solver {
  ...
  ll calcScore(PressG &pressG, vll &A) {
    ll ret = 0;
    vll seen(N,0);

    rep(i, 0, exampleVertices.size()) {
      // pqの初期値
      priority_queue<tll, vt, greater<tll>> pq;
      rep(j, 0, pressG.G.size()) {
        int flag = 0;
        for (auto cu : pressG.clusterToVertices[j]) {
          if (pressG.isContainVertice(j, exampleVertices[i])) flag = 1;
          if (flag) break;
        }
        if (flag) {
          pq.push({1, j, -1});
      ...
      ll score = -1;
      rep(j, 0, pressG.G.size()) {
        if (!pressG.isContainVertice(j, exampleVertices[i])) continue;
        bool flag = 0;
        flag = (dp[j] < dist);
        if (flag) {
          bestV = j;
  ...
  void solve(vector<Event> &ans, vll &A) {
    ...
    rep(i, 0, N) {
      ...
      chmax(maX, x[i]), chmin(maY, y[i]);
    }
    vll score(4, INF);
    vll yosumi(4, 0);
    rep(i, 0, N) {
      if (chmin(score[0], abs(miX - x[i]) + abs(miY - y[i]))) {
        yosumi[0] = i;
      }
      if (chmin(score[1], abs(maX - x[i]) + abs(maY - y[i]))) {
        yosumi[1] = i;
      }
      if (chmin(score[2], abs(miX - x[i]) + abs(maY - y[i]))) {
        yosumi[2] = i;
      }
      if (chmin(score[3], abs(maX - x[i]) + abs(miY - y[i]))) {
        yosumi[3] = i;
      }
    }
    rep(i, 0, 4) exampleVertices.push_back(yosumi[i]);
    exampleVertices.push_back(yosumi[0]);
    ...
    while (1) {
      if (0.5 < elaTimes(startTime)) break;
      loop++;
      vvll atTree(N);
      vll atA;
      constructTree(atTree);
      initConstructA(atA, atTree);
      PressG atPressG;
      atPressG.constructPressG(atA);

      ll score = calcScore(atPressG, atA);
      // cerr << score;
...
// local6 <- local4
// 複数の木を構成して、最良なものを採用する。端の頂点4つで試す