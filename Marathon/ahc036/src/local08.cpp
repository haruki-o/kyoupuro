...
struct Solver {
  ...
  void calcScoreDfs(ll c, ll p, vll &visited, vvll &tree, vvll &newTree, vll &A, map<pll, ll> &edgeScore, int switchV, int type) {
    if (visited[c]) return;
    A.push_back(c);
    visited[c] = 1;
    if (c == switchV) type = 1;
    ...
    if (type == 0) {
      for (auto cu : tree[c]) {
        calcScoreDfs(cu, c, visited, tree, newTree, A, edgeScore, switchV, type);
      }
    }
    if (type == 1) {
      vp vec;
      ...
      gSort(vec);
      for (auto cu : vec) {
        calcScoreDfs(cu.second, c, visited, tree, newTree, A, edgeScore, switchV, type);
      }
    }
  }

  ll calcScore(vll &A, vvll &tree, map<ll, ll> &pathScore, map<ll, ll> &hash, vll &table) {
    ll ret = 0;
    rep(i, 0, N - PATH_SIZE + 1) {
      ll atHash = 0;
      rep(j, 0, PATH_SIZE) {
        atHash = (atHash ^ table[A[i + j]]);
      }
      ret += pathScore[atHash];
    }
    return ret;
  }
  // ll calcScore(vll &A, vvll &tree, map<pll, ll> &edgeScore) {
  //   ll ret = 0;
  //   rep(i, 0, N - 1) {
  //     ret += edgeScore[{min(A[i],A[i+1]),max(A[i],A[i+1])}];
  //   }
  //   return ret;
  // }
  ...
  void constructTree(vll &A, vvll &tree) {
    ...
    map<ll, ll> hash;
    map<ll, ll> pathScore;
    rep(i, 0, N) table[i] = rand() % ((ll)1 << 60);
    ...
    rep(i, 0, T - 1) {
      ...
      rep(j, 0, candi.size()) {
        ...
        rep(k, 0, path.size() - PATH_SIZE + 1) {
          ll atHash = 0;
          rep(l, 0, PATH_SIZE) {
            atHash = (atHash ^ table[path[k + l]]);
          }
          pathScore[atHash]++;
        }
      }
    ...
    // for(auto cu: pathScore)if(cu.second > 4)cerr << cu.second << " ";
    ...
    vll visited(N, 0);
    dfs(root, -1, visited, tree, A, edgeScore);
...
#endif
    ll loop = 0;
    // ll bestScore = calcScore(A, tree, edgeScore);
    ll bestScore = calcScore(A, tree, pathScore, hash, table);
    while (1) {
      loop++;
      if (SATime < elaTimes(startTime)) break;
      ...
      visited.assign(N, 0);
      vvll newTree(N);
      calcScoreDfs(root, -1, visited, tree, newTree, _A, edgeScore, idx, 0);
      ll score = calcScore(_A, newTree, pathScore, hash, table);
      // ll score = calcScore(_A, newTree, edgeScore);

      if (_A.size() != N) cerr << "out" << endl;
      ll sum = 0;
      rep(i, 0, N) sum += newTree[i].size();
      // cerr << sum << endl;
      if (sum != 1198) cerr << "tree out" << endl;
  ...
  void initConstructA(vll &A, vvll &tree) {
    ...
    ll addNum = LA - A.size();
    if (LB >= 20) chmin(addNum, 50);
    if (LB >= 15) chmin(addNum, 100);
    if (LB >= 10) chmin(addNum, 200);
    if (LB >= 7) chmin(addNum, 300);

    ll idx = rand() % (A.size() - 1);
    rep(i, 0, A[i]) {
      int flag = 1;
      for (auto cu : g[A[idx]]) {
        if (cu == A[idx + 1]) flag = 0;
      }
      if (flag) break;
      idx = (idx + 1) % (A.size() - 1);
    }
    vll addVertices;
    addVertices.push_back(A[idx]);
    addNum--;
    vll st;
    st.push_back(A[idx]);
    map<ll, ll> used;
    while (addNum > 0 && st.size() != 0) {
      ll cu = st.back();
      st.pop_back();
      if (used.count(cu) == 0) {
        addVertices.push_back(cu);
        addNum--;
        used[cu] = 1;
      } else continue;
      ll _idx = rand() % g[cu].size();
      rep(j, 0, g[cu].size()) {
        ll to = g[cu][_idx];
        _idx = (_idx + 1) % g[cu].size();
        if (used.count(to) > 0) continue;
        st.push_back(to);
      }
    }
    ll addIndex = idx + 1;
    for (ll cu : addVertices) {
      A.insert(A.begin() + addIndex, cu);
      addIndex++;
    }
...
// local8 <- local7
// 木を変更し、pathScore[hash]が最大になるように山登り(ほぼ変わりなし)
//t[i] -> t[i+1]の最短pathに含まれる長さPATH_SIZEのパスを+1とする。