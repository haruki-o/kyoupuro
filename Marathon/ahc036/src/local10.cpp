...
struct PressG {
  ...
  // PressGを構築する
  void lightConstructPressG(vll &A) {
    vp ok;
...
struct Solver {
  ...
  void calcScoreDfs(ll c, ll p, vll &visited, vvll &tree, vvll &newTree, vll &A, map<pll, ll> &edgeScore, int switchV, int type) {
    ...
    if (type == 1) {
      ...
      for (auto cu : g[c]) {
        ...
        // if (c == switchV || rand() % 4 == 0) vec.push_back({rand(), cu});
  ...
  ll calcScore(PressG &pressG) {
    ll ret = 0;
    rep(i, 0, T - 1) {
      // ダイクストラ法
      queue<ll> qu;
      vll dp(pressG.G.size(), INF);
      vll ok(pressG.G.size(), 0);
      rep(j, 0, pressG.G.size()) {
        if (pressG.isContainVertice(j, t[i])) {
          dp[j] = 0;
          qu.push(j);
        }
        if (pressG.isContainVertice(j, t[i + 1])) {
          ok[j] = 1;
        }
      }
  ...
  void constructTree(vll &A, vvll &tree) {
    ...
    PressG pressG;
    pressG.lightConstructPressG(A);
    ll bestScore = calcScore(pressG);
    ...
    while (1) {
      ...
      PressG _pressG;
      _pressG.lightConstructPressG(_A);
      ll score = calcScore(_pressG);
    ...
    // cerr << "constructTree() SA:loop: " << loop << endl;
  }
...
// local10 <- local8
// 木を変更し、軽量化したダイクストラが最大になるように山登り(ほぼ変わりなし)