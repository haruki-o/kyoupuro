...
struct Solver {
  void dfs(ll c, ll p, vll &visited, vvll &tree, vll &A, map<pll, ll> &edgeScore, int type = 0) {
    if (visited[c]) return;
    A.push_back(c);
    visited[c] = 1;
    if (p != -1) {
    ...
    gSort(vec);
    for (auto cu : vec) {
      dfs(cu.second, c, visited, tree, A, edgeScore, type);
    }
  }
  ...
  ll calcScore(vll &A, vvll &tree, map<pll, ll> &edgeScore) {
    ...
    rep(i, 0, A.size()) {
      rep(j, 0, LB) {
        if (A.size() <= i + j + 1) break;
        if (gMat[A[i + j]][A[i + j + 1]]) {
          ret += edgeScore[{min(A[i + j], A[i + j + 1]), max(A[i + j], A[i + j + 1])}];
        } else break;
      }
    }
    return ret;
  ...
  void constructTree(vll &A, vvll &tree) {
    ...
    vll _A;
    visited.assign(N, 0);
    dfs(root, -1, visited, tree, _A, edgeScore, 1);
    ll addNum = LA - A.size();
    ...
    rep(i, 0, T) {
      ...
      rep(j, 0, pressG.G.size()) {
        ...
        if (dp[j] == dist) {
          ...
          if (score == atScore) {
            ll atSameDistScore = INF;
            for (ll cu : pressG.G[j]) {
              if (i + 1 != T) {
                chmin(atSameDistScore, gDist[t[i + 1]][cu]);
              }
            }
            if (chmin(sameDistScore, atSameDistScore)) {
              flag = 1;
            }
          }
...
// local12 <- local8, 9 (ほぼ変わりなし, そのままだとTLE)
// constructTree()
//    木を変更し、各iに対してsum(A[i:i+LB-1])が最大になるように山登り
// initConstructA()
//    削除し,constructTree()に併合
// ininAns, ダイクストラ
//    次のクラスターに行くとき、bestVを次のt[i+1]に近くなるようなクラスターを採用する