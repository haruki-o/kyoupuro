...
struct Solver {
  ...
  void solve(vector<Event> &ans, vll &A) {
    ...
    // cerr << "constructTree(): " << elaTimes(startTime) << endl;

    auto startTime1 = system_clock::now();
    PressG pressG;
    pressG.constructPressG(A);
    // cerr << "costructPressG(): " << elaTimes(startTime1) << endl;
    ...
    // cerr << "initAns(): " << elaTimes(startTime2) << endl;
  }
};
...
// local13 <- local9 (微増)
// constructTree()
//   A[]に追加するときA[i],A[i+1]が接続してないとき,pathを追加
// initConstructA()
//    削除し,constructTree()に併合
// initAns()
//    辺の距離が1なのでダイクストラをbfsに変更し、大幅な時間短縮