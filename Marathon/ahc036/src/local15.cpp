...
struct Solver {
  ...
  // クラスターclustIdxから圧縮前の頂点toを含むクラスターへの最短距離
  ll distScore(PressG &pressG, ll clustIdx, ll to, ll tIdx) {
    queue<ll> qu;
    ...
    rep(j, 0, pressG.G.size()) {
      if (!pressG.isContainVertice(j, t[tIdx])) continue;
      ll atScore = dp[j];
      ll idx = tIdx;
      ...
      if (chmin(score, atScore)) bestV = j;
    }
  ...
  void initAns(vll &A, vector<Event> &ans, PressG &pressG) {
    ...
    rep(i, 0, T) {
      ...
      score = INF;
      if (candi.size() >= 2) {
        rep(j, 0, min((ll)candi.size(), (ll)3)) {
          ll atScore = distScore(pressG, j, t[i + 1], i + 1);
          if (chmin(score, atScore)) {
            bestV = candi[j];
          }
        }
...
// local15 <- local14 (微増)
// initAns()
//    次に選ぶクラスターを距離が同じときt[i+1]に行く場合の距離の最小値で選びました
//    時間調整を行いました