...
struct Solver {
  ...
  void modify(vll &A, vvll &tree) {
    ll ran = rand() % 5;
    if (ran == 0) {
      ll idx = rand() % A.size();
      ll si = rand() % (A.size() - idx);
      vll store;
      rep(i, idx, idx + si) store.push_back(A[i]);
      A.erase(A.begin() + idx, A.begin() + idx + si);
      idx = rand() % A.size();
      rep(i, idx, idx + si) {
        A.insert(A.begin() + i, store[i - idx]);
      }
    }
    if (1 <= ran && ran <= 2) {
      ll idx1 = rand() % A.size();
      ll idx2 = (idx1 + rand() % A.size()) % A.size();
      ll mi = min(idx1, idx2);
      ll ma = max(idx1, idx2);
      reverse(A.begin() + idx1, A.begin() + idx2);
    }
    // if (0) {
    if (3 <= ran) {
      vll seen(N, 0);
