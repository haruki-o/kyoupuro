...
struct PressG {
  ...
  // SA用の軽量化PressGを構築する
  void constructLightPressG(vll &A) {
    vp ok;
...
struct Solver {
  ...
  ll calcAns(PressG &pressG) {
    ll ret = 0;
    ll preCluster;
    ll preVertice = 0;
    double stat = 0;
    map<pll, ll> storeScore;
    vp model;
    for (ll x = 100; x < 1000; x += 100) {
      for (ll y = 100; y < 1000; y += 100) {
        model.push_back({x, y});
      }
    }
    rep(i, 0, T) {
      ll model1 = 0, model2 = 0;
      ll modelScore = INF;
      if (i != 0) {
        rep(j, 0, model.size()) {
          auto _ = model[j];
          if (chmin(modelScore, abs(_.first - x[t[i - 1]]) + abs(_.second - y[t[i - 1]]))) model1 = j;
        }
        modelScore = INF;
        rep(j, 0, model.size()) {
          auto _ = model[j];
          if (chmin(modelScore, abs(_.first - x[t[i]]) + abs(_.second - y[t[i]]))) model2 = j;
        }
        ll mi = min(model1, model2);
        ll ma = max(model1, model2);
        model1 = mi;
        model2 = ma;
        if (storeScore.count({model1, model2}) > 0) {
          ret += storeScore[{model1, model2}];
          continue;
        }
      }
      // cerr << endl << "t[] index: " << i << endl;
      ...
      storeScore[{model1, model2}] = score;
      ret += score;
  ...
  void solve(vector<Event> &ans, vll &A) {
    ...
    while (1) {
      ...
      PressG _pressG;
      _pressG.constructLightPressG(_A);
      ll score = calcAns(_pressG);
