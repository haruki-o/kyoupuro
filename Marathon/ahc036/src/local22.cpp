...
struct Solver {
  ...
  void constructTree(vll &A, vvll &tree, int initFlag = 0) {
    ll root = 0;
    ll _x = 500, _y = 500;
    if (!initFlag) _x = rand() % 1000, _y = rand() % 1000;
    vll ok(N, 0);
    rep(i, 0, T) ok[t[i]] = 1;
    rep(i, 0, N) {
      if (!ok[i]) continue;
        ...
        if (gMat[A[i]][A[i + 1]]) {
          ...
          while (!qu.empty()) {
            ...
            for (ll _to : g[_cu]) {
              ...
              if (edgeScore[{min(_cu, _to), max(_cu, _to)}] == 0) continue;
              qu.push({_to, _cu});
              dp[_to] = dp[_cu] + 1;
              pre[_to] = _cu;
            }
...
// local22 <- local21 (微増)
// initAns()
// 同じスコアのときbestVを最小のgDist[][]が存在するものに遷移
// (バグ修正)