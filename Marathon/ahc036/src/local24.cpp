...
struct Solver {
  ...
  void solve(vector<Event> &ans, vll &A) {
    ...
    while (1) {
      ...
      vvll _tree(N);
      vll _A;
      constructTree(_A, _tree, loop);
      PressG _pressG;
      _pressG.constructPressG(_A);
      vector<Event> _ans;
      initAns(A, _ans, _pressG);
      ...
      if (chmin(bestScore, score)) {
        ...
        A = _A;
        ans.clear();
        rep(i, 0, N) tree[i] = _tree[i];
      }
      loop++;
    }
    PressG __pressG;
    __pressG.constructPressG(A);
    addTree(A, tree, __pressG);
    PressG _pressG;
    _pressG.constructPressG(A);
    initAns(A, ans, _pressG);
...
void checkAns(vector<Event> &ans, vll &A) {
  ...
  if (idx != T) cerr << "not all goal" << endl;
}
