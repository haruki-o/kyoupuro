...
struct Solver {
  void dfs(ll c, ll p, vll &visited, vvll &tree, vll &A, int type = 0) {
    ...
    for (auto cu : g[c]) {
      if (type == 0) {
        if (edgeScore[{min(c, cu), max(c, cu)}] != 0) vec.push_back({edgeScore[{min(c, cu), max(c, cu)}], cu});
      }
      if (type == 1) vec.push_back({rand() % 10 * edgeScore[{min(c, cu), max(c, cu)}], cu});
      if (type == 2) vec.push_back({edgeScore[{min(c, cu), max(c, cu)}], cu});
    }
  ...
  void constructTree(vll &A, vvll &tree, int initFlag = 0) {
    ll root = 0;
    ll _x = 500, _y = 500;
    if (initFlag != 0) _x = rand() % 1000, _y = rand() % 1000;
    vll ok(N, 0);
    rep(i, 0, T) ok[t[i]] = 1;
    rep(i, 0, N) {
      if (!ok[i]) continue;
      if (abs(x[i] - _x) + abs(y[i] - _y) < abs(x[root] - _x) + abs(y[root] - _y)) root = i;
    }
    ...
    while (1) {
      vp ok;
      rep(i, 0, N) if (seen[i] == 1) ok.push_back({dp[i], i});
      gSort(ok);
      int flag = 0;
      rep(i, 0, ok.size()) {
        priority_queue<tll, vt> pq;
        pq.push({0, ok[i].second, -1});
        vll _dp = dp;
        vll pre(N, -1);
        while (!pq.empty()) {
          auto at = pq.top();
          pq.pop();
          ll cu = get<1>(at);
          if (_dp[cu] != 0) continue;
          _dp[cu] = get<0>(at);
          if (cu == ok[i].second) _dp[cu] = 1;
          pre[cu] = get<2>(at);
          for (ll to : g[cu]) {
            if (_dp[to] != 0) continue;
            ll score = edgeScore[{min(cu, to), max(cu, to)}];
            if (score == 0) continue;
            pq.push({get<0>(at) + score, to, cu});
          }
        }
        ll _at = 0;
        ll score = -1;
        ...
        rep(j, 0, path.size()) {
          if (j != 0) {
            ...
            A.push_back(path[j]);
            seen[path[j]] = 1;
            dp[path[j]] = _dp[path[j]];
          }

          if (j != path.size() - 1) {
            tree[path[j]].push_back(path[j + 1]);
            tree[path[j + 1]].push_back(path[j]);
          }
        }
      }
      if (!flag) break;
    ...
    cerr << A.size() << " ";
    ll sum = 0;
    rep(i, 0, N) sum += seen[i];
    cerr << "sum " << sum << endl;
    int flag = 0;
    UnionFind uf(N);
    rep(i, 0, N) {
      for (ll to : tree[i]) uf.unite(i, to);
    }
    rep(i, 0, T) {
      rep(j, i + 1, T) if (!uf.same(t[i], t[j])) flag = 1;
    }
    if (flag) cerr << "don't unite" << endl;

    if (!flag) return;
    vll visited(N, 0);
    rep(i, 0, N) tree[i].clear();
    A.clear();
    dfs(root, -1, visited, tree, A, 2);
  }

  void addTree(vll &A, vvll &tree, PressG &pressG) {
    vt add;
    rep(i, 0, T) {
      ...
      ll sta = i == 0 ? 0 : t[i - 1];
      ll addScore = score - gDist[sta][t[i]] / LB;
      add.push_back({addScore, sta, t[i]});
    }
    gSort(add);
    ...
    rep(i, 0, add.size()) {
      ...
      for (auto cu : used) {
        ...
        ll _cu2 = get<1>(add[i]);
        ll _to2 = get<2>(add[i]);
        ll dist = abs(x[_cu1] - x[_cu2]) + abs(y[_cu1] - y[_cu2]);
        dist += abs(x[_to1] - x[_to2]) + abs(y[_to1] - y[_to2]);
        if (dist < 20) flag = 1;
        swap(_cu1, _to1);
        dist = abs(x[_cu1] - x[_cu2]) + abs(y[_cu1] - y[_cu2]);
        dist += abs(x[_to1] - x[_to2]) + abs(y[_to1] - y[_to2]);
        if (dist < 20) flag = 1;
      }
      if (flag) continue;
      used.push_back({get<1>(add[i]), get<2>(add[i])});
  ...
  void initConstructEdgeScore() {
    ...
    rep(i, 0, T - 1) {
      ...
      rep(j, 0, candi.size()) {
        ll at = candi[j];
        _edgeScore[{min(at, t[i + 1]), max(at, t[i + 1])}] += 1 / (double)candi.size();
        // edgeScore[{min(at, t[i + 1]), max(at, t[i + 1])}]++;
        while (at != -1) {
          ll to = pre[at];
          if (to != -1) _edgeScore[{min(at, to), max(at, to)}] += 1 / (double)candi.size();
          // if (to != -1) edgeScore[{min(at, to), max(at, to)}]++;
          at = to;
        }
      }
    }
    for (auto cu : _edgeScore) {
      edgeScore[cu.first] = cu.second + 1;
    }
  }
  ...
  void initAns(vll &A, vector<Event> &ans, PressG &pressG) {
    ...
    rep(i, 0, T) {
      ...
      rep(j, 0, pressG.G.size()) {
        ...
        ll atScore = dp[j];
        ll idx = i;
        while (pressG.isContainVertice(j, t[idx])) {
          idx++;
          atScore--;
          if (idx == T) break;
        }
        if (score == atScore && i + 1 != T) {
          ll mi = INF;
          for (ll ver : pressG.clusterToVertices[j]) {
            chmin(mi, gDist[ver][t[i + 1]]);
          }
          candi.push_back({mi, j});
        }
        if (chmin(score, atScore)) {
          candi.clear();
          ll mi = INF;
          for (ll ver : pressG.clusterToVertices[j]) {
            if (i + 1 != T) chmin(mi, gDist[ver][t[i + 1]]);
          }
          candi.push_back({mi, j});
        }
      }
      bestV = candi[0].second;
  ...
  void solve(vector<Event> &ans, vll &A) {
    ...
    while (1) {
      ...
      vll _A;
      vector<Event> _ans;
      constructTree(_A, newTree, loop);
      PressG _pressG;
      _pressG.constructPressG(_A);

      // editConstructEdgeScore(_A, _ans, _pressG);

      vector<Event> __ans;
      addTree(_A, newTree, _pressG);
      PressG __pressG;
      __pressG.constructPressG(_A);
      initAns(_A, __ans, __pressG);
      ll score = calcAns(__ans);
      // cerr << score << endl;
...
int main() {
  ...
  gDist.resize(N);
  rep(i, 0, N) gDist[i].assign(N, INF);
  rep(i, 0, N) rep(j, 0, N) if (gMat[i][j]) gDist[i][j] = 1;
  rep(k, 0, N) {
    rep(i, 0, N) {
      rep(j, 0, N) chmin(gDist[i][j], gDist[i][k] + gDist[k][j]);
    }
  }
...
// local25 <- local23 (微増)
// initAns()
//  A[]が足りないとき、t[i]->t[i+1]の実スコア - 距離/LBが大きい順にPATHを追加, 同じ方向の時追加しない。
//  edgeScore()の計算を1/(double)candi.size()を追加するようにした